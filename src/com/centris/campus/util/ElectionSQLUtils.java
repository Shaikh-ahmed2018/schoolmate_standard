...
public class ElectionSQLUtils {
	...
	public static final String savePopUpData ="INSERT INTO `campus_election_category_setting`(`electionCategoryId`,`electionCategory`,`priority`,`classId`,`genderWise`,`houseWise`,`classWise`,`nominationFor`,nominationLevel,`electionSettingID`,`accyearID`,`groupID`)VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
	public static final String updatePopUpData="UPDATE  `campus_election_category_setting` SET `electionCategory`=?,`priority`=?,`classId`=?,`genderWise`=?,`houseWise`=?,`classWise`=?,`nominationFor`=?,nominationLevel=?,`electionSettingID`=?,`accyearID`=?,`groupID`=? WHERE `electionCategoryId`=?";
