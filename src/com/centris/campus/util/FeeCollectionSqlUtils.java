...
*/	
    ...
    public static final String GET_FEECOLLECTION_DEDDER ="select cstd.student_imgurl_var imgurl,cstd.classdetail_id_int,cstd.specilization, concat(s.student_fname_var,' ',s.student_lname_var) studentname,s.student_admissionno_var,a.acadamic_year,c.classdetails_name_var,sec.classsection_name_var,cstd.isTransferInMiddle from campus_acadamicyear a,campus_classdetail c,campus_classsection sec,campus_student s LEFT OUTER JOIN campus_scholorship schol ON s.student_id_int=schol.studentId LEFT OUTER JOIN campus_fee_concessiondetails conc ON conc.concessionid=schol.concessionId left join campus_student_classdetails cstd on s.student_id_int=cstd.student_id_int where  c.classdetail_id_int=cstd.classdetail_id_int and c.locationId=cstd.locationId and sec.classsection_id_int=cstd.classsection_id_int and s.student_id_int=? and cstd.fms_acadamicyear_id_int=a.acadamic_id and a.acadamic_id=? and cstd.locationId=?";
	...
	public static final String GET_COLLECTION_UPDATE_CNT = "select feeCollectionCode from campus_fee_collection where admissionNo=? and accYear=? and termcode=?";
	public static final String INSERT_FEE_COLLECTION = "insert into campus_fee_collection(feeCollectionCode,admissionNo,accYear,termcode,is_paid,totalamount,actualamount,due_amount,advance_amount,paidDate,createdby,createdtime,amount_paid,balance_amount,fineAmount,paymentParticulars,paymentMode,dd_cheque_date,bank_name,concessionAmt,paidByExcel,chln_no) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	public static final String INSERT_FEE_COLLECTION_DETAILS = "insert into campus_fee_collectiondetails(feeCollectionCode, feeCode,feeAmount,finalFeeAmtcollected,outstandingfee,concessionPercent,consfeeAmount,feepaiddate)values(?,?,?,?,?,?,?,?)";
	public static final String INSERT_FEE_RECIEPT = "insert into campus_fee_reciept(feeCollectionCode, feeCode,feeAmount,finalFeeAmtcollected,outstandingfee,concessionPercent,consfeeAmount,receipt_no,feeType)values(?,?,?,?,?,?,?,?,?)";
	...
	public static final String GET_TRANSPORT_FEECOLLECTION_DEDDER = "select DISTINCT cstd.classdetail_id_int,cstd.specilization,cst.StageId,cst.start_month srtm ,cst.end_month endm,cst.NumberOfMonth, concat(s.student_fname_var,' ',s.student_lname_var) studentname,s.student_admissionno_var,a.acadamic_year,c.classdetails_name_var,sec.classsection_name_var from campus_acadamicyear a,campus_classdetail c,campus_classsection sec,campus_student s left outer join campus_fee_concessiondetails conc on s.student_scholorship_var=conc.concessionid join campus_student_transportdetails cst on s.student_id_int=cst.student_id_int join campus_student_classdetails cstd on s.student_id_int=cstd.student_id_int AND cst.fms_acadamicyear_id_int=cstd.fms_acadamicyear_id_int where  c.classdetail_id_int=cstd.classdetail_id_int and c.locationId=cstd.locationId and sec.classsection_id_int=cstd.classsection_id_int and cst.student_id_int=? and cstd.fms_acadamicyear_id_int=a.acadamic_id and cstd.locationId=? and cstd.fms_acadamicyear_id_int=?";
	...
	//defenderFee 
	public static final String getDefaulterFeeList ="SELECT CONCAT(cs.student_fname_var, ' ',student_lname_var) AS studentName,cs.`student_admissionno_var`,cs.`student_id_int`,l.Location_Name,cc.classdetails_name_var,ccs.classsection_name_var,t.`termname`,t.`termid`,csc.`classdetail_id_int`,csc.fms_acadamicyear_id_int,csc.locationId FROM `campus_student` cs JOIN `campus_student_classdetails` csc ON cs.student_id_int = csc.student_id_int AND cs.`locationId`=csc.`locationId` and cs.`student_status_var`='active' JOIN `campus_location` l ON l.`Location_Id`=csc.`locationId` AND l.`isActive`='Y' JOIN `campus_acadamicyear` ca ON ca.`acadamic_id`=csc.`fms_acadamicyear_id_int` AND ca.`isActive`='Y' JOIN `campus_classstream` stm ON stm.`classstream_id_int`=csc.`fms_classstream_id_int` AND stm.`isActive`='Y' JOIN campus_classdetail cc ON csc.`classdetail_id_int`=cc.classdetail_id_int AND csc.locationId=cc.locationId AND cc.`isActive`='Y'  JOIN campus_classsection ccs ON csc.classsection_id_int=ccs.classsection_id_int AND ccs.`isActive`='Y' LEFT JOIN `campus_parentchildrelation` cpc ON cs.`student_admissionno_var`=cpc.`stu_addmissionNo`   JOIN `campus_fee_termdetails` t ON t.`locationId`=cs.`locationId` WHERE cs.`student_id_int`  NOT IN (SELECT `admissionNo` FROM `campus_fee_collection` WHERE `accYear`=? AND `termcode`= ?) AND cs.`locationId`= ? AND csc.classdetail_id_int LIKE ? AND ccs.classsection_id_int LIKE ? AND t.`termid`=? AND csc.fms_acadamicyear_id_int = ? ORDER BY studentName,cc.classdetails_name_var,ccs.classsection_name_var";  
