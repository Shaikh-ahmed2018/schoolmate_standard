...
public class CommunicateUtilConstants {
	...
	public static final String GET_CLASS_DETAILS = "select classdetail_id_int,classdetails_name_var from campus_classdetail where `isActive`='Y' order by classdetail_id_int";
	...
	public static final String GET_STUDENT_MEETING ="SELECT stu.student_fname_var,cls.classdetails_name_var, sec.classsection_name_var FROM campus_student stu LEFT JOIN campus_student_classdetails cscd ON cscd.student_id_int = stu.student_id_int LEFT JOIN campus_classdetail cls ON cls.classdetail_id_int = cscd.classdetail_id_int AND cls.`locationId`=cscd.`locationId`LEFT JOIN campus_classsection sec ON sec.classsection_id_int = cscd.classsection_id_int AND sec.`locationId`=cscd.`locationId` JOIN `campus_location` cl ON cl.`Location_Id`=cscd.`locationId` AND cl.`isActive`='Y' WHERE stu.student_id_int=? AND cscd.fms_acadamicyear_id_int=? AND cscd.locationId=?";
