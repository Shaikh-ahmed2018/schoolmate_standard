...
public class FeeExcelUploadUtil {
	...
	public Map<String, Object> getExcelData(File fileURL, FormFile file,String extension) {
		...
		try{
			...
			for (int i = 10; i <= sheetcountlist.size() - 10;) {
	...
	public static String getfeesettingCode(String termid,String accyear,String studentails, UserLoggingsPojo userLoggingsVo){
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
	...
	public String checkStudentAdmin(String admissionno, String locationid, UserLoggingsPojo userLoggingsVo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		...
		try{
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement("SELECT student_id_int FROM campus_student WHERE student_admissionno_var = ? and locationId=?");
