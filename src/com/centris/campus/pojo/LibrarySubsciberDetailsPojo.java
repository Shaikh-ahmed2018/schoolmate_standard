package com.centris.campus.pojo;

public class LibrarySubsciberDetailsPojo {
	
	private String SubscriberNo;
	private String AdmissionNo;
private String staffRegId;
	private String classId;
	private String sectionId;
	private String Name;
	private String Gender;
	private String Standard;
	private String PhoneNo;
	private String Email_ID;
	private String Address;
	private String Date;
	private String Division;
	private String MobileNo;
	private String requestedby;
	private String startedby;
	private String cardNoAmount;
	private String checkNoAmount;
	
	private String teacher;
	private String student;
	private String other;
	
	//----subscriberDetails:::Save
	private String adminssionNo;
	private String studentIdHidden;
	private String accId;
	private String staffID;
	
	private String depositType;
	private String enterAmount;
	private String cardNo;
	private String checkNo;
	
	private String department;
	private String designation;
	
	private String status;
	private String paymentDate;
	
	private String otherUserAddr;
	private String otherUserName;
	private String otherUserGender;
	private String otherUserContact;
	private String otherUserEmail;
	
	private String locationName;
	private String suscriberType;
	
	private String staffid;
	private String deptId;
	private String desgId;
	
	private String select;
	private String searchTextVal;
	private String search;
	private String orderBy;
	
	private String entryId;
	private String statement; 
	
	private String accessionNo;
	private String subscriber_id;
	private String author;
	private String bookTitle;
	private String from_date; 
	private String subscriberName;  
	private String studentId;
	private String staffId;
	private int slno;
	
	private String libLoc;
	
	private String log_audit_session;

	public String getLog_audit_session() {
		return log_audit_session;
	}
	public void setLog_audit_session(String log_audit_session) {
		this.log_audit_session = log_audit_session;
	}
	
	public String getLibLoc() {
		return libLoc;
	}
	public void setLibLoc(String libLoc) {
		this.libLoc = libLoc;
	}
	public int getSlno() {
		return slno;
	}
	public void setSlno(int slno) {
		this.slno = slno;
	}
	public String getRequestedby() {
		return requestedby;
	}
	public void setRequestedby(String requestedby) {
		this.requestedby = requestedby;
	}
	public String getStartedby() {
		return startedby;
	}
	public void setStartedby(String startedby) {
		this.startedby = startedby;
	}
	public String getStaffid() {
		return staffid;
	}
	public void setStaffid(String staffid) {
		this.staffid = staffid;
	}
	public String getDeptId() {
		return deptId;
	}
	public void setDeptId(String deptId) {
		this.deptId = deptId;
	}
	public String getDesgId() {
		return desgId;
	}
	public void setDesgId(String desgId) {
		this.desgId = desgId;
	}

	public String getDepartment() {
		return department;
	}
	public void setDepartment(String department) {
		this.department = department;
	}
	public String getDesignation() {
		return designation;
	}
	public void setDesignation(String designation) {
		this.designation = designation;
	}
	
	public String getClassId() {
		return classId;
	}
	public void setClassId(String classId) {
		this.classId = classId;
	}
	public String getSectionId() {
		return sectionId;
	}
	public void setSectionId(String sectionId) {
		this.sectionId = sectionId;
	}
	public String getSuscriberType() {
		return suscriberType;
	}
	public void setSuscriberType(String suscriberType) {
		this.suscriberType = suscriberType;
	}
	public String getLocationName() {
		return locationName;
	}
	public void setLocationName(String locationName) {
		this.locationName = locationName;
	}
	public String getTeacher() {
		return teacher;
	}
	public void setTeacher(String teacher) {
		this.teacher = teacher;
	}
	public String getStudent() {
		return student;
	}
	public void setStudent(String student) {
		this.student = student;
	}
	public String getOther() {
		return other;
	}
	public void setOther(String other) {
		this.other = other;
	}
	public String getAdminssionNo() {
		return adminssionNo;
	}
	public void setAdminssionNo(String adminssionNo) {
		this.adminssionNo = adminssionNo;
	}
	public String getStudentIdHidden() {
		return studentIdHidden;
	}
	public void setStudentIdHidden(String studentIdHidden) {
		this.studentIdHidden = studentIdHidden;
	}
	public String getStaffID() {
		return staffID;
	}
	public void setStaffID(String staffID) {
		this.staffID = staffID;
	}
	public String getDepositType() {
		return depositType;
	}
	public void setDepositType(String depositType) {
		this.depositType = depositType;
	}
	public String getEnterAmount() {
		return enterAmount;
	}
	public void setEnterAmount(String enterAmount) {
		this.enterAmount = enterAmount;
	}
	public String getCardNo() {
		return cardNo;
	}
	public void setCardNo(String cardNo) {
		this.cardNo = cardNo;
	}
	public String getCheckNo() {
		return checkNo;
	}
	public void setCheckNo(String checkNo) {
		this.checkNo = checkNo;
	}
	public String getStatus() {
		return status;
	}
	public void setStatus(String status) {
		this.status = status;
	}
	public String getPaymentDate() {
		return paymentDate;
	}
	public void setPaymentDate(String paymentDate) {
		this.paymentDate = paymentDate;
	}
	public String getOtherUserAddr() {
		return otherUserAddr;
	}
	public void setOtherUserAddr(String otherUserAddr) {
		this.otherUserAddr = otherUserAddr;
	}
	public String getOtherUserName() {
		return otherUserName;
	}
	public void setOtherUserName(String otherUserName) {
		this.otherUserName = otherUserName;
	}
	public String getOtherUserGender() {
		return otherUserGender;
	}
	public void setOtherUserGender(String otherUserGender) {
		this.otherUserGender = otherUserGender;
	}
	public String getOtherUserContact() {
		return otherUserContact;
	}
	public void setOtherUserContact(String otherUserContact) {
		this.otherUserContact = otherUserContact;
	}
	public String getOtherUserEmail() {
		return otherUserEmail;
	}
	public void setOtherUserEmail(String otherUserEmail) {
		this.otherUserEmail = otherUserEmail;
	}
	
	

	
	public String getSubscriberNo() {
		return SubscriberNo;
	}
	public void setSubscriberNo(String subscriberNo) {
		SubscriberNo = subscriberNo;
	}
	public String getAdmissionNo() {
		return AdmissionNo;
	}
	public void setAdmissionNo(String admissionNo) {
		AdmissionNo = admissionNo;
	}
	public String getName() {
		return Name;
	}
	public void setName(String name) {
		Name = name;
	}
	public String getGender() {
		return Gender;
	}
	public void setGender(String gender) {
		Gender = gender;
	}
	public String getStandard() {
		return Standard;
	}
	public void setStandard(String standard) {
		Standard = standard;
	}
	public String getPhoneNo() {
		return PhoneNo;
	}
	public void setPhoneNo(String phoneNo) {
		PhoneNo = phoneNo;
	}
	public String getEmail_ID() {
		return Email_ID;
	}
	public void setEmail_ID(String email_ID) {
		Email_ID = email_ID;
	}
	public String getAddress() {
		return Address;
	}
	public void setAddress(String address) {
		Address = address;
	}
	public String getDate() {
		return Date;
	}
	public void setDate(String date) {
		Date = date;
	}
	public String getDivision() {
		return Division;
	}
	public void setDivision(String division) {
		Division = division;
	}
	public String getMobileNo() {
		return MobileNo;
	}
	public void setMobileNo(String mobileNo) {
		MobileNo = mobileNo;
	}
	public String getAccId() {
		return accId;
	}
	public void setAccId(String accId) {
		this.accId = accId;
	}
	public String getStaffRegId() {
		return staffRegId;
	}
	public void setStaffRegId(String staffRegId) {
		this.staffRegId = staffRegId;
	}
	public String getSelect() {
		return select;
	}
	public void setSelect(String select) {
		this.select = select;
	}
	public String getSearchTextVal() {
		return searchTextVal;
	}
	public void setSearchTextVal(String searchTextVal) {
		this.searchTextVal = searchTextVal;
	}
	public String getSearch() {
		return search;
	}
	public void setSearch(String search) {
		this.search = search;
	}
	public String getOrderBy() {
		return orderBy;
	}
	public void setOrderBy(String orderBy) {
		this.orderBy = orderBy;
	}
	public String getEntryId() {
		return entryId;
	}
	public void setEntryId(String entryId) {
		this.entryId = entryId;
	}
	public String getStatement() {
		return statement;
	}
	public void setStatement(String statement) {
		this.statement = statement;
	}
	public String getAccessionNo() {
		return accessionNo;
	}
	public void setAccessionNo(String accessionNo) {
		this.accessionNo = accessionNo;
	}
	public String getSubscriber_id() {
		return subscriber_id;
	}
	public void setSubscriber_id(String subscriber_id) {
		this.subscriber_id = subscriber_id;
	}
	public String getAuthor() {
		return author;
	}
	public void setAuthor(String author) {
		this.author = author;
	}
	public String getBookTitle() {
		return bookTitle;
	}
	public void setBookTitle(String bookTitle) {
		this.bookTitle = bookTitle;
	}
	public String getFrom_date() {
		return from_date;
	}
	public void setFrom_date(String from_date) {
		this.from_date = from_date;
	}
	public String getSubscriberName() {
		return subscriberName;
	}
	public void setSubscriberName(String subscriberName) {
		this.subscriberName = subscriberName;
	}
	public String getStudentId() {
		return studentId;
	}
	public void setStudentId(String studentId) {
		this.studentId = studentId;
	}
	public String getStaffId() {
		return staffId;
	}
	public void setStaffId(String staffId) {
		this.staffId = staffId;
	}
	public String getCheckNoAmount() {
		return checkNoAmount;
	}
	public void setCheckNoAmount(String checkNoAmount) {
		this.checkNoAmount = checkNoAmount;
	}
	public String getCardNoAmount() {
		return cardNoAmount;
	}
	public void setCardNoAmount(String cardNoAmount) {
		this.cardNoAmount = cardNoAmount;
	}
	
}
