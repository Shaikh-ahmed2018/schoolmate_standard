...
public class StageFeeSetupAction extends DispatchAction{
	...
	public ActionForward getEditedFeeSetupDetails(ActionMapping mapping,
		...
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StageFeeSetupAction : getEditedFeeSetupDetails Starting");
		...
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StageFeeSetupAction : getEditedFeeSetupDetails Ending");
	...
	public ActionForward insertFeeAmount(ActionMapping mapping,
		...
		try {
			...
			classSetupList=new StageFeeSetupBD().getStageFeeSetupDetails(currentaccyear,userLoggingsVo);
		...
		try {
			...
			if (searchTerm == null || "".equalsIgnoreCase(searchTerm)) {
				...
				classSetupList = new StageFeeSetupBD()
						.getStageFeeSetupDetails(currentaccyear,userLoggingsVo);
	...
	public ActionForward downloadstagefeesetupPDF(ActionMapping mapping,
			...
			try {
				...
				if (searchTerm == null || "".equalsIgnoreCase(searchTerm)) {
					...
					classSetupList = new StageFeeSetupBD()
							.getSearchStageFeeSetupDetails(searchTerm,currentaccyear,userLoggingsVo);
	...
	public ActionForward schoolListStage(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : schoolListStage Starting");
		try {
			...
			list= new TransportBD().schoolListStage(loc,custdetails);
			JSONObject object = new JSONObject();
			object.put("accYearList", list);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : schoolListStage Ending");
		return null;
	}
	
	public ActionForward getStage(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : getStage Starting");
		try {
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			...
			list= new TransportBD().getStage(accyear,loc,userLoggingsVo);
			//request.setAttribute("vehicleDetails", status); 
			JSONObject object = new JSONObject();
			object.put("stageList", list);
			response.getWriter().print(object);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			e.printStackTrace();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : getStage Ending");
		return null;
	}
	
	public ActionForward schoolList(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : schoolList Starting");
		try {
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			...
			String loc = request.getParameter("location");
			ArrayList<ExaminationDetailsVo> list=new ArrayList<ExaminationDetailsVo>();
			list= new TransportBD().schoolList(accyear,loc,userLoggingsVo);
			//request.setAttribute("vehicleDetails", status); 
			JSONObject object = new JSONObject();
			object.put("accYearList", list);
			response.getWriter().print(object);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : schoolList Ending");
		return null;
	}
	
	public ActionForward getPreAcc(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : getPreAcc Starting");
		try {
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			...
			list= new TransportBD().getPreAcc(accyear,loc,userLoggingsVo);
			//request.setAttribute("vehicleDetails", status);
			JSONObject object = new JSONObject();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : getPreAcc Ending");
		return null;
	}
	
	public ActionForward addStageWiseAmount(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StageFeeSetupAction : addStageWiseAmount Starting");
		try {
			...
			String accyear = request.getParameter("accyear");
			String loc = request.getParameter("locationid");
			String[] stageid=request.getParameterValues("stageid[]");
			String[] amount=request.getParameterValues("amount[]");
			String user=HelperClass.getCurrentUserID(request);
			String status= new TransportBD().addStageWiseAmount(accyear,loc,stageid,amount,user,userLoggingsVo,log_audit_session);
			//request.setAttribute("vehicleDetails", status); 
			JSONObject object = new JSONObject();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StageFeeSetupAction : addStageWiseAmount Ending");
		return null;
	}

	public ActionForward getPresentAndNextAccyear(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : getPresentAndNextAccyear Starting");
		try {
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			...
			list= new TransportBD().getPresentAndNextAccyear(accyear,loc,userLoggingsVo);
			//request.setAttribute("vehicleDetails", status);
			JSONObject object = new JSONObject();
			object.put("list", list);
			response.getWriter().print(object);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : getPresentAndNextAccyear Ending");
		return null;
	}
	
	public ActionForward schoolListRoutMaster(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : schoolListRoutMaster Starting");
		try {
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			
			String loc = request.getParameter("location");
			ArrayList<ExaminationDetailsVo> list=new ArrayList<ExaminationDetailsVo>();
			list= new TransportBD().schoolListRoutMaster(loc,userLoggingsVo);
			JSONObject object = new JSONObject();
			object.put("accYearList", list);
			response.getWriter().print(object);
		} catch (Exception e) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TransportAction : schoolListRoutMaster Ending");
		return null;
	}
}
