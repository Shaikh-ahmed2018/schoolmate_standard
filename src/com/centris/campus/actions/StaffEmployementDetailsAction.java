...
private static Logger logger = Logger.getLogger(StaffEmployementDetailsAction.class);
	...
	public synchronized ActionForward getStaffEmployementEntry(ActionMapping mapping,
		...
		try {
			...
			StaffEmployementVo stafflist=new StaffEmployementBD().getStaffEmployementEntry(teachercode,academic_year,userLoggingsVo);
			
			request.setAttribute("teachercode", teachercode);
			
			request.setAttribute("Stafflist", stafflist);
			request.setAttribute("BankList", HelperClass.getBankdetails(userLoggingsVo));
			...
			request.setAttribute("salaryupdate", "salaryupdate");
...
*/				emp_sal_vo.setSpecialallowance(specialallowance);
	...
	public synchronized ActionForward saveStaffSalaryDetails(ActionMapping mapping,
		...
		try {
			...
			String academic_year = (String) request.getSession(false).getAttribute("current_academicYear");
			String location=staffForm.getHlocationId();
			...
			staffForm.setLog_audit_session(log_audit_session);
			staffForm.setUserLoggingsvo(userLoggingsVo);
			
			String status=new StaffEmployementBD().saveStaffSalaryDetails(staffForm,userLoggingsVo);
	...
	public ActionForward downloadStaffEmploymentDetailsPDF(ActionMapping mapping,
			...
			try {
				...
				Details = new TeacherRegistrationBD().getAllTeacherDetails(custdetails,obj);
	...
	public  ActionForward getPTdetails(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getPTdetails Starting");
		try {
			...
			double profeesionaltax=new StaffEmployementDaoImpl().getPTdetails(location,monthlyamount,userLoggingsVo);

			JSONObject object=new JSONObject();
			object.put("profeesionaltax", profeesionaltax);
			response.getWriter().print(object);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getPTdetails Ending");

		return null;
	}
	
	public  ActionForward getCalculationMonth(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getCalculationMonth Starting");
		try {

			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			
			
			String emloyeeid=request.getParameter("emloyeeid");
			String academicyaer=request.getParameter("academicyaer");
			
			double employeecurrenttaxablemonth=new StaffEmployementDaoImpl().getCalculationMonth(emloyeeid,academicyaer,userLoggingsVo);

			JSONObject object=new JSONObject();
			object.put("employeecurrenttaxablemonth", employeecurrenttaxablemonth);
			response.getWriter().print(object);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getCalculationMonth Ending");

		return null;
	}
	
	
	public  ActionForward getBranchName(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getBranchName Starting");
		try {
			...
			JSONObject object=new JSONObject();
			object.put("BranchList", HelperClass.getBankBranch(bankId, userLoggingsVo));
			response.getWriter().print(object);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getBranchName Ending");

		return null;
	}
	
	public  ActionForward getIFSCCode(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getIFSCCode Starting");
		try {
			...
			JSONObject object=new JSONObject();
			object.put("IfscCode", HelperClass.getIFSCCode(branchId, userLoggingsVo));
			response.getWriter().print(object);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StaffEmployementDetailsAction : getIFSCCode Ending");
