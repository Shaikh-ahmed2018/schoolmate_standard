...

{
	...
	public ActionForward EditingForAdmissionApproval(ActionMapping mapping,
		...
		try {
			...
			ArrayList<ReportMenuVo> classList = new ReportsMenuBD()
					.getClassesByStream("%%",userLoggingsVo);
	...
	public ActionForward UpdatingFirstLevelAdmissionApproval(
		...
		try {
			...
			list = obj.getAdmisssionProcessingListDetails(userLoggingsVo);
	...
	public ActionForward InsertTemporaryStudentRegistration(
		...
		try {
			...
			parentform.setCustid(userLoggingsVo.getCustId());
			...
			StudentIDGenerator = IDGenerator.getPrimaryKeyID("campus_temporary_admisssion_details",userLoggingsVo);
			try {
				String extension="";
			...
			String Enquiryid=parentform.getEnquiryid();
			ParentRequiresAppointmentVO result=detailsBD.getSchoolDetails(Enquiryid,userLoggingsVo);
			request.setAttribute("result",result);

			String details = detailsBD.InsertTemporaryStudentRegistration(parentform,log_audit_session,userLoggingsVo);
		...
		try {
			...
			try {
				...
				if (fileName.length() < 2 && fileName != null) {
					...
					imagepath = getServlet().getServletContext().getRealPath("")+ "\\FIles\\StudentImageTempStorage"+ "/"+secform.getStudentfirstName()+"_"+StudentIDGenerator + "." + extension;
			...
			secform.setLog_audit_session(log_audit_session);

			
			String details = secformBD.InsertSecadmissionform(secform,userLoggingsVo);
	...
	public ActionForward DeleteParentRequiresAppointment(ActionMapping mapping,
		...
		try {
			...
			List<ParentRequiresAppointmentVO> list = new ArrayList<ParentRequiresAppointmentVO>();
			list = detailsBD.getAdmisssionProcessingListDetails(userLoggingsVo);
			request.setAttribute("TemporaryAdmissionDetailsList", list);
	...
	public ActionForward EditingForCalledAdmissionDetails(
		...
		try {
			...
			ParentRequiresAppointmentVO ckeck = detailsBD
					.EditingForCalledAdmissionDetails(detailsVo,userLoggingsVo);
	...
	public ActionForward UpdatingCalledForEvaluationStatus(
		...
		try {
			...
			list = obj.CalledForEvaluationList(userLoggingsVo);
			request.setAttribute("TemporaryAdmissionDetailsList", list);
	...
	public ActionForward EditingForConfirmingAdmissionDetails(
		...
		try {
			...
			request.setAttribute("AccYearList", accYearList);
			ArrayList<ReportMenuVo> classList = new ReportsMenuBD().getClassesByStream("%%",userLoggingsVo);
			request.setAttribute("classList", classList);
	...
	public ActionForward UpdatingFinalApprovalAdmissionStatus(
		...
		try {
			...
			details.setCustid(userLoggingsVo.getCustId());
			details.setTemporary_id(temporary_id);
			details.setCurrent_user(userid);
			...
			String ckeck = detailsBD.UpdatingFinalApprovalAdmissionStatus(details,userLoggingsVo);
			...
			list = obj.FinalAdmisssionList(userLoggingsVo);
	...
	*/
	...
	byte[] bytes = JasperRunManager.runReportToPdf(jasperreport,
			parameters, JDBCConnection.getConnection(userLoggingsVo));
	...
	public ActionForward Thirdadmissionformat(
		...
		try {
			...
			String details = detailsBD.Insertthirdadmissionformat(parentform,userLoggingsVo);
	...
	public ActionForward Issuedformlist(ActionMapping mapping,
		...
		try {
			...
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
             ...
             ArrayList<Issuedmenuvo> issuedList = new TemporaryRegBD().getissuedforms("%%",userLoggingsVo,schoolLocation+","+accYear);
	...
	public ActionForward insertapprovedlist(ActionMapping mapping,
		...
		try {
				...
				String approveSt = null;
				System.out.println("othereason  "+othereason);
				if(othereason == null || othereason.equalsIgnoreCase("")){
					approveSt = request.getParameter("approveSt");
				}else{
					approveSt = othereason;
				}
		        ...
		        String approvedList = new TemporaryRegBD().insertapprovedlist(idList,reason,othereason,mobile_number,log_audit_session,pojo,locname+","+userId+","+approveSt);
			    JSONObject jsonObject = new JSONObject();
				jsonObject.put("status", approvedList);
				response.getWriter().print(jsonObject);
				
			    request.setAttribute("approvedList", approvedList);
	            ArrayList<Issuedmenuvo> issuedList = new TemporaryRegBD().getissuedforms("%%",pojo,schoolLocation+","+accYear);
	            request.setAttribute("issuedList", issuedList);
	...
	public ActionForward getApprovedList(ActionMapping mapping,
		...
		try {
			...
			String accYear=(String) request.getSession(false).getAttribute("current_academicYear");
			String schoolLocation = (String) request.getSession(false).getAttribute("current_schoolLocation");
			ArrayList<Issuedmenuvo> getapprvedlist = new TemporaryRegBD().getApprovedForms("%%",userLoggingsVo,accYear,schoolLocation);
	...
	public ActionForward insertrejectedlist(ActionMapping mapping,
		...
		try {
			...
			if(otherrsn == null || otherrsn.trim().equalsIgnoreCase("")){
				approveSt = request.getParameter("approveSt");
			}else{
				approveSt = otherrsn;
			}
			
			String rejectedList = new TemporaryRegBD().insertrejectedList(idLists,rejectreason,otherrsn,log_audit_session,userLoggingsVo,approveSt);
	...
	public ActionForward insertcancelledlist(ActionMapping mapping,
		...
		try {
				...
				if(canreason == null || canreason.trim().equalsIgnoreCase("")){
					approveSt = request.getParameter("approveSt");
				}else{
					approveSt = canreason;
				}
				
		        String cancelledlist = new TemporaryRegBD().insertcancelledlist(idList,reason,canreason,log_audit_session,userLoggingsVo,approveSt);
	...
	public ActionForward getCancelledList(ActionMapping mapping,
		...
		try {
			...
			String schoolLocation = (String) request.getSession(false).getAttribute("current_schoolLocation");
			UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
            ArrayList<Issuedmenuvo> getcancelledlist = new TemporaryRegBD().getCancelledForms("%%",userLoggingsVo,accYear,schoolLocation);
		...
		try {
			...
			try {
				...
				if (fileName.length() < 2 && fileName != null) {
					...
					secform.setImageString(getServlet().getServletContext().getRealPath("/")+ "FIles/STUDENTIMAGES/"+"noImage.png");
			...
			try {
				...
				if (fileName.length() < 2 && fileName != null) {
					...
					secform.setMarkImageFile(getServlet().getServletContext().getRealPath("/")+ "FIles/STUDENTIMAGES/"+"noImage.png");

				} else if(extension.equalsIgnoreCase("jpg")){
					imagepath = getServlet().getServletContext().getRealPath("")+ "\\FIles\\StudentImageTempStorage"+ "/"+"marksheet"+secform.getStudentfirstName()+"_"+StudentIDGenerator + "." + extension;
					System.out.println("imagepath is "+imagepath);
					realPath = imagepath.substring(imagepath.indexOf("FIles\\"));
					System.out.println("realPath is "+realPath);
					outputStream = new FileOutputStream(new File(imagepath));
	...
	public ActionForward getAdmissionRegistrationDetails(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			List<ParentRequiresAppointmentVO> admissionList = new ParentRequiresAppointmentDELEGATE().getAdmissionRegDetails(appointmentVo,userLoggingsVo);
