...

{
	...
	public ActionForward getClassesByStream(ActionMapping mapping,
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in RegistrationOfUser : getClassesByStream Starting");
		try {
			...
			ArrayList<ReportMenuVo> classesList = new ReportsMenuBD()
					.getClassesByStream(streamId,userLoggingsVo);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in RegistrationOfUser : getClassesByStream Ending");
	...
	public ActionForward InsertNewRegistrationUser(ActionMapping mapping,
		...
		try {
            ...
            System.out.println("Insertion Action:" + request.getParameter("locationid"));
			...
			registrationform.setLog_audit_session(log_audit_session);
			registrationform.setLocation_id(request.getParameter("locationid"));
			registrationform.setAccyear(Acyearid);
			
			String registration = new NewRegistrationBD().InsertNewRegistrationUser(registrationform,userLoggingsVo);
	...
	public ActionForward saveparentsubmittingdetailstoschool(
		...
		try {
			...
			String registration = new NewRegistrationBD()
					.saveparentsubmittingdetailstoschool(registrationform,userLoggingsVo);
