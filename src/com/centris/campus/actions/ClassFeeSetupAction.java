...
public class ClassFeeSetupAction extends DispatchAction {
	...
	public ActionForward getEditedFeeSetupDetails(ActionMapping mapping,
		...
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ClassFeeSetupAction : getEditedFeeSetupDetails Starting");
		...
		try {
		    ...
		    custSchoolInfo=HelperClass.getCustSchoolInfo(userLoggingsVo,loc_id);
			...
			request.setAttribute("locationname", HelperClass.getSchoolName(loc_id, userLoggingsVo)+","+custSchoolInfo.getLocationname());
			
			try {
				
				getHeading=feeSetupBD.getHeading(feeSetupPojo,userLoggingsVo);
				
				if(getHeading.get(0).getSpecilazationCount()==0){
					
				String location = loc_id;	
				getActiveFeeMasterSetupDetails = feeSetupBD.getAllFees(feeSetupPojo,location,userLoggingsVo);
				...
				forward=MessageConstants.FEE_SETUP_ENTRY;
					...
					getSpecializationFeeMasterSetupDetails = feeSetupBD.getApprovedFees(feeSetupPojo,userLoggingsVo);
		...
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ClassFeeSetupAction : getEditedFeeSetupDetails Ending");
	...
	public ActionForward insertApproveFees(ActionMapping mapping,
		...
		try {
			...
			message = new ClassFeeSetupBD().insertApproveFees(approvefeelist,userLoggingsVo);
			...
			feeSetupPojos.setCustid(userLoggingsVo.getCustId());
			request.setAttribute("getApprovedFeeMasterSetupDetails",
					new ClassFeeSetupBD().getApprovedFees(feeSetupPojos,userLoggingsVo));
			request.setAttribute("getActiveFeeMasterSetupDetails",
					new ClassFeeSetupBD().getAllFees(feeSetupPojos,location,userLoggingsVo));
	...
	public ActionForward deleteFees(ActionMapping mapping, ActionForm form,
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ClassFeeSetupAction : deleteFees Starting");
		String location = (String) request.getSession(false).getAttribute("current_schoolLocation");	
		...
		try {
			...
			feeSetupPojo.setTerm(term);
			feeSetupPojo.setSpecialization(specCode);
			feeSetupPojo.setCustid(userLoggingsVo.getCustId());
			boolean status = new ClassFeeSetupBD().deleteFees(feeSetupPojo,userLoggingsVo);

			request.setAttribute("getActiveFeeMasterSetupDetails",
					new ClassFeeSetupBD().getAllFees(feeSetupPojo,location,userLoggingsVo));
			request.setAttribute("getApprovedFeeMasterSetupDetails",
					new ClassFeeSetupBD().getApprovedFees(feeSetupPojo,userLoggingsVo));
		...
		try {
		  ...
		  UserLoggingsPojo userLoggingsVo = (UserLoggingsPojo) request.getSession(false).getAttribute("token_information");
			...
			for (int i = 0; i < feecode_array.length; i++) {
				...
				feeSetupPojo.setNewStuFees(Double.parseDouble(feecode_array[i].split("-")[3]));
				feeSetupPojo.setClassId(classid);
				feeSetupPojo.setAcadamicYear(acadamicYear);
				feeSetupPojo.setLocation_id(loc_id);
				...
				feeSetupPojo.setLog_audit_session(log_audit_session);
				feeAmountList.add(feeSetupPojo);
			}
			
			flag = new ClassFeeSetupBD().insertFeeAmount(feeAmountList,userLoggingsVo);
		...
		try {
			...
			*/
			...
			if (searchTerm == null || "".equalsIgnoreCase(searchTerm)) {
				...
				classSetupList = new ClassFeeSetupBD().getFeeSetupDetails(currentaccyear,userLoggingsVo);
	...
	public ActionForward downloadclassfeesetupPDF(ActionMapping mapping,
			...
			/*	String currentaccyear = request.getSession(false)
				...
				if (searchTerm == null || "".equalsIgnoreCase(searchTerm)) {
					...
					classSetupList = new ClassFeeSetupBD()
							.getFeeSetupDetails(currentaccyear,userLoggingsVo);

				} else {

					classSetupList = new ClassFeeSetupBD()
							.getSearchFeeSetupDetails(searchTerm, currentaccyear,userLoggingsVo);
	...
	public ActionForward getClassFeeSetupByJs(ActionMapping mapping,
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ClassFeeSetupAction : getClassFeeSetupByJs Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ClassFeeSetupAction : getClassFeeSetupByJs Ending");
