...
public class CommunicationSettingsDaoImpl implements CommunicationSettingsDao {
	...
	public ArrayList<UpcomingRemarksVO> getRemarksListDetailsDao(UserLoggingsPojo userLoggingsVo) {
		...
		try {
			...
			pstmt = connection.prepareStatement(CommunicateUtilConstants.GET_ALL_STUDENT_REMARKS_DETAILS);
		...
		try {
			...
			pstmt = connection.prepareStatement(CommunicateUtilConstants.GET_ALL_TEACHER_REMARKS_DETAILS);
	...
	public List<StreamDetailsVO> getStreamListDetailsDao(UserLoggingsPojo custid) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
	...
	public List<ClassPojo> getClassListDetailsDao(ClassPojo pojo,UserLoggingsPojo userLoggingsVo) {
		...
		try {
			...
			pstmt = conn.prepareStatement(CommunicateUtilConstants.GET_CLASS_DETAILS);
	...
	public ArrayList<SectionPojo> getSectionListDetailsDao(String[] categoryval) {
		...
		try {
			...
			pstmt = conn.prepareStatement(CommunicateUtilConstants.GET_SECTION_DETAILS);
	...
	public ArrayList<StudentInfoVO> getStudentListDetailsDao(
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection();
			pstmt = conn.prepareStatement(CommunicateUtilConstants.GET_STUDENT_DETAILS);
	...
	public ArrayList<UpcomingRemarksVO> getRemarkListDetailsDao(
			UpcomingRemarksVO remrakvo,UserLoggingsPojo userLoggingsVo) {
	...
	public String saveMeetingDetailsDao(MeetingPojo pojo) {
		...
		try {
			...
			pstmt = conn.prepareStatement(CommunicateUtilConstants.SAVE_MEETING);
	...
	public ArrayList<UpcomingBdayVo> searchBadyListDetailsDao(
			UpcomingBdayVo bdayvo,UserLoggingsPojo userLoggingsVo) {
		...
		try {
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement(CommunicateUtilConstants.SEARCH_ALL_BDAY_DETAILS);
			...
			rs = pstmt.executeQuery();
             ...
             while (rs.next()) {
				...
				if (person.contains("TEA")) {
					...
					pstmt1 = conn.prepareStatement(CommunicateUtilConstants.GET_TEACHER_BDAY_WISHES);
	...
	public ArrayList<LstmsUpcomingMeetingVO> searchMeetingListDetailsDao(
		...
		try {
			...
			connection = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = connection.prepareStatement(CommunicateUtilConstants.SEARCH_ALL_MEETING_DETAILS);
			...
			rs = pstmt.executeQuery();
              ...
              while(rs.next()){
				...
				if (teacherid.contains("TEA")) {
					...
					pstmt1 = connection.prepareStatement(CommunicateUtilConstants.GET_TEACHER_MEETING);
	...
	public ArrayList<UpcomingBdayVo> getBdayListDetailsDao(UserLoggingsPojo userLoggingsVo) {
	...
	public ArrayList<LstmsUpcomingMeetingVO> getMeetingListDetailsDao(UserLoggingsPojo userLoggingsVo) {
