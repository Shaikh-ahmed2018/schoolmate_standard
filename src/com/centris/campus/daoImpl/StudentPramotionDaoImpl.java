...
public class StudentPramotionDaoImpl implements StudentPramotionDao {
	...
	public List<StudentPramotionVO> getStudentData(String acadamicYear,
		...
		try {
			...
			ArrayList<String> list = HelperClass.getNextAccYearDetails(acadamicYear,userLoggingsVo);
	...
	public StudentPramotionVO insertStudentPromotion(
		...
		for (int i = 0; i < studentidval.length; i++) {
			...
			try {
				...
				pstmt = conn.prepareStatement(StudentPromotionSqlUtil.GET_ADMISSIONNO_VAL);
				...
				while (rs.next()) {
					...
					pstmt1 = (PreparedStatement) JDBCConnection
							.getStatement(StudentPromotionSqlUtil.INSERT_STUDENT_PROMOTION,studentPramotionPOJO.getCustid());
							...
							for (int l = k; l <= k; l++) {
								for (int m = l; m <= l; m++) {
									for (int n = m; n <= m; n++) {
										...
										vo.setCustid(studentPramotionPOJO.getCustid());
										
										List<StudentRegistrationVo> studentList = new StudentRegistrationDaoImpl().searchStudentResult(vo,userLoggingsVo);

										JSONArray a = new JSONArray(studentList);
										//ln(a);
										
										if (checkApplicationNo(studentList.get(0).getStudentAdmissionNo(),
												toacadamicyearval[k],userLoggingsVo)) {

											promotionvo.setStname(studentList.get(0).getStudentFirstName());
											promotionvo.setAdmissionno(studentList.get(0).getStudentAdmissionNo());
											
											promotionvo.setSectionname(studentList.get(0).getSectionnaem());
											promotionvo.setClassname(studentList.get(0).getClassname());
											promotionvo.setAcadamicyear(studentList.get(0).getAcademicYear());
...
*/
												...
												if (status
													...
													pstmt2 = (PreparedStatement) JDBCConnection
															.getStatement(StudentPromotionSqlUtil.UPDATE_STUDENT,studentPramotionPOJO.getCustid());
	...
	public boolean checkApplicationNo(String admissionNumber, String studentId,UserLoggingsPojo userLoggingsVo)
			throws Exception {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
	...
	public String saveStudentRegistration(List<StudentRegistrationVo> registration,
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentPramotionDaoImpl : saveStudentRegistration Starting");
				...
				if (registration.get(0).getPrimaryPerson().equals("father")) {
					...
					if (registration.get(0).getFatheremailId() == null) {
						new SendMail().sendMailtoChild(registration.get(0)
								...
								fatherName.concat(parentUserName),null);
					}
					...
					if (registration.get(0).getMotheremailId() == null) {
						new SendMail().sendMailtoChild(registration.get(0)
								...
								motherName.concat(parentUserName),null);
					}
				} else {
					...
					if (registration.get(0).getGuardianemailId() == null) {
						new SendMail().sendMailtoChild(registration.get(0)
								...
								guardianName.concat(parentUserName),
								guardianName.concat(parentUserName),null);
					}
				}
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentPramotionDaoImpl : saveStudentRegistration Ending");
		return status;
	}
	...
	public ArrayList<StudentPramotionVO> getpromotionslist(UserLoggingsPojo userLoggingsVo) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentPramotionDaoImpl: getpromotionslist  Starting");
		...
		try {
			...
			//ln("GET_ENQUIRY_STUDENT_DATA"+pstmt);
			resultsetano = pstmt.executeQuery();
			while (resultsetano.next()) {
				StudentPramotionVO studentPramotionVO = new StudentPramotionVO();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentPramotionDaoImpl: getpromotionslist   Ending");
	...
	public ArrayList<StudentPramotionVO> getpromotionssearchlist(String className, String sectionName) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentPramotionDaoImpl: getpromotionssearchlist Ending");
		return studentlist;
	}
	...
	public List<ClassPromotionList> getClassListForPromotion(String currentYear,UserLoggingsPojo userLoggingsVo) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentPramotionDaoImpl: getClassListForPromotion Ending");
