...
public class AcadamicYearPlanDaoImpl implements AcadamicYearPlanDao{
	...
	public String insertAcadamicYearPlan(AcadamicYearPlanPOJO acadamicYearPlanPOJO) {
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(acadamicYearPlanPOJO.getCustid());
			...
			if(acadamicYearPlanPOJO.getLocationId().equalsIgnoreCase("all")){
				locationList = new  LocationBD().getLocationDetails(acadamicYearPlanPOJO.getCustid());
				//(locationList.size());
			}
			if(locationList.size()!=0){
				for(int j=0;j<locationList.size();j++){
					String planId= IDGenerator.getPrimaryKeyID("campus_accyearplan");
					ps_insertPlan.setString(1,planId);
					...
					ps_insertPlan.setString(12,acadamicYearPlanPOJO.getVenue());
					ps_insertPlan.setString(13,locationList.get(j).getLocation_id());
					count = ps_insertPlan.executeUpdate();
					if(count>0){
						HelperClass.recordLog_Activity(acadamicYearPlanPOJO.getLog_audit_session(),"Settings","Activities","Insert",ps_insertPlan.toString(),acadamicYearPlanPOJO.getCustid());
					}
				}
			}
			else{
				...
				ps_insertPlan.setTimestamp(11,HelperClass.getCurrentTimestamp());
				ps_insertPlan.setString(12,acadamicYearPlanPOJO.getVenue());
				ps_insertPlan.setString(13,acadamicYearPlanPOJO.getLocationId());
				count = ps_insertPlan.executeUpdate();
				if(count>0){
					HelperClass.recordLog_Activity(acadamicYearPlanPOJO.getLog_audit_session(),"Settings","Activities","Insert",ps_insertPlan.toString(),acadamicYearPlanPOJO.getCustid());
				}
	...
	public ArrayList<AcadamicYearPlanVO> getSearchDetails(AcadamicYearPlanPOJO acadamicYearPlanPOJO) {
		...
		try {
			...
			ps_getEvent.setString(6,acadamicYearPlanPOJO.getSerachText()+"%");
	...
	public ArrayList<AcadamicYearPlanVO> getAllEventDetails(AcadamicYearPlanPOJO acadamicYearPlanPOJO) {
		...
		try {
			...
			//("GET_ALL_ACADAMICYEAR_PLANS::"+ps_getEvent);
			...
			while(rs_getEvent.next()){
				...
				eventdetailsVO.setVenue_details(rs_getEvent.getString("venue_details"));
				eventdetailsVO.setLocationName(rs_getEvent.getString("Location_Name"));
				//("Title::"+rs_getEvent.getString("Title"));
				eventlist.add(eventdetailsVO);
			}
	...
	public String updateAcadamicYearPlan(AcadamicYearPlanPOJO acadamicYearPlanPOJO) {
		...
		try {
			...
			ps_insertPlan.setString(10,acadamicYearPlanPOJO.getVenue());
			ps_insertPlan.setString(11,acadamicYearPlanPOJO.getLocationId());
			ps_insertPlan.setString(12,acadamicYearPlanPOJO.getEventid());
			int count = ps_insertPlan.executeUpdate();

			//(ps_insertPlan);

			if(count>0){
				HelperClass.recordLog_Activity(acadamicYearPlanPOJO.getLog_audit_session(),"Settings","Activities","Update",ps_insertPlan.toString(),acadamicYearPlanPOJO.getCustid());
				status = MessageConstants.TRUE;
			}
	...
	public String validateAcadamicYearPlan(AcadamicYearPlanPOJO acadamicYearPlanPOJO) {
		...
		try {
			...
			//("ps_validatePlan::::"+ps_validatePlan);
	...
	public String validateAcadamicYearPlanUpdate(AcadamicYearPlanPOJO acadamicYearPlanPOJO) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in AcadamicYearPlanDaoImpl: validateAcadamicYearPlanUpdate : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in AcadamicYearPlanDaoImpl: validateAcadamicYearPlanUpdate: Ending");
