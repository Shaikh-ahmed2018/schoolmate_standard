...
public class ElectionDaoImpl implements ElectionDao {
	...
	public List<PageFilterVo> getAccyear( String academicYear) {
			...
			/*inserted orderby in query*/
					psmt2=conn.prepareStatement("SELECT * FROM campus_acadamicyear where acadamic_id like  ? ORDER BY `acadamic_year` ASC");
					psmt2.setString(1, academicYearId);
	...
	public String saveGroupdetails(String accyear, String locid,String groupname) {
		...
		try{/*{cheking groupname exists or not*/
			...
			if(count3==0){
				key = IDGenerator.getPrimaryKeyID("campus_election_group_settings");
			...
			psmt2.setString(1,IDGenerator.getPrimaryKeyID("campus_election_school_setting"));
...
public String saveElectionDetails(ElectionPojo pojo) {
	...
	if(count1==0){
		key = IDGenerator.getPrimaryKeyID("campus_election_election_setting");
...
public String electionCategoryAddPopUp(ElectionPojo pojo) {
	...
	//	ElectionVo result = new ElectionVo();
		...
		if(count1==0){
		key = IDGenerator.getPrimaryKeyID("campus_election_category_setting");
		...
		if(count>0){
			...
			result = "dupPri"; // checking groupname exists or not
				...
				psmt.setString(12, pojo.getElectionCategoryId());
...
public ArrayList<ElectionVo> getNominationRegistrationList(String academicYear,String groupName, String electionTitle) {
	...
	try{
		conn = JDBCConnection.getSeparateConnection();
		psmt = conn.prepareStatement("SELECT * FROM `campus_election_category_setting` WHERE `electionSettingID` like ? AND `accyearID` like ? AND `groupID` like  ?");
...
public ArrayList<ElectionVo> getUpdateElectionCategory(String categoryId) {
	...
	try{
			conn=JDBCConnection.getSeparateConnection();
			pstmt=conn.prepareStatement("SELECT `electionCategory`,`priority`,`classId`,`genderWise`,`houseWise`,`classWise`,`nominationFor`,`nominationLevel` FROM `campus_election_category_setting` WHERE `electionCategoryId`=?");
			...
			if(count2==0){// if not duplicate booth
				...
				if(count3==0){
					...
					if(count1==0){
						...
						if(count5==0){
						key = IDGenerator.getPrimaryKeyID("campus_election_booth_setting");
...
public String savePollingMachineDetails(ElectionPojo pojo) {
	...
	try{
		...
		while(rs0.next()){
			...
			psmt2=conn.prepareStatement("SELECT COUNT(*) FROM campus_election_polling_machine_setting WHERE boothNameId_int=?");
			psmt2.setString(1, pojo.getBoothName());
	...
	//else part for insert
		else 
		{
			...
			if(count0==0){
			String key1 = IDGenerator.getPrimaryKeyID("campus_election_polling_machine_setting");
...
public String checkMachicneActivation(String localIp) {
	...
	int count=0;
 	try{
		conn = JDBCConnection.getSeparateConnection();
		psmt=conn.prepareStatement("SELECT COUNT(*) FROM campus_election_polling_machine_setting WHERE systemIp_var=? AND status='Y'");
...
public String getMachicneActivation(ElectionPojo object) {
	...
	int count2=0;
 	try{
		...
		if(count0 > 0){
		psmt=conn.prepareStatement("SELECT boothNameId_int FROM campus_election_booth_setting WHERE centralSystemIp=? AND accyearID=? AND groupID=? AND electionSettingID=?");
		psmt.setString(1, object.getCentralSystemIp());
		psmt.setString(2, object.getAccid());
		psmt.setString(3, object.getGroupId());
		psmt.setString(4, object.getElectionTitleId());
		System.out.println("psmtpsmtpsmtpsmt:::"+psmt);
		rs=psmt.executeQuery();
			while (rs.next()) {
				boothId = rs.getString("boothNameId_int");

				psmt2 = conn
						.prepareStatement("SELECT COUNT(*) FROM polling_verification_voter_list WHERE voterId=? AND academicYearId=? AND electionGroupId=? AND electionTitleId=?");
				...
				if (count1 == 0) {
					psmt1 = conn.prepareStatement("UPDATE campus_election_polling_machine_setting SET status='Y',activationFor=? WHERE boothNameId_int=? AND status='N'");
					psmt1.setString(1,object.getStudentId() + "," + object.getClassId()+ "," + object.getHouseId());
					psmt1.setString(2, boothId);
...
public List<StudentRegistrationVo> studentSearchbyadmissionNo(
	...
	try {
		...
		conn = JDBCConnection.getSeparateConnection();
		pstmObj = conn.prepareStatement(StudentRegistrationSQLUtilConstants.STUDENT_SEARCH_BY_ADMISSION);
	...
	try {
		...
		while(rs_categoryList.next()){
			...
			if(rs_categoryList.getString("houseWise").equalsIgnoreCase("NO"))
			{
				...
				if(rs_categoryList.getString("genderWise").equalsIgnoreCase("BOTH") || rs_categoryList.getString("genderWise").equalsIgnoreCase("NO")){
				ps_cadidateList=conn.prepareStatement("SELECT  CONCAT(cs.student_fname_var, ' ',student_lname_var) AS studentName,cenr.*, cc.classdetails_name_var,cc.classdetail_id_int,ccs.classsection_name_var, ccs.classsection_id_int,csc.student_imgurl_var,csc.`student_id_int`,CASE WHEN chs.housename IS NULL THEN 'No House' ELSE chs.housename END housename FROM campus_election_nomiation_registration cenr LEFT JOIN campus_student cs ON cs.student_admissionno_var = cenr.admissionNo LEFT JOIN campus_student_classdetails csc ON cs.student_id_int = csc.student_id_int LEFT JOIN campus_classdetail cc ON (csc.classdetail_id_int=cc.classdetail_id_int AND csc.locationId=cc.locationId) LEFT JOIN campus_classsection ccs ON csc.classsection_id_int=ccs.classsection_id_int LEFT JOIN campus_house_settings chs ON csc.student_house WHERE electionCategoryId=? AND isApproved='APPROVED' AND csc.student_house=? ORDER BY studentName");
				ps_cadidateList.setString(1, rs_categoryList.getString("electionCategoryId"));
				ps_cadidateList.setString(2, activationFor.split(",")[2]);
				}
				else{
					ps_cadidateList=conn.prepareStatement("SELECT  CONCAT(cs.student_fname_var, ' ',student_lname_var) AS studentName,cenr.*, cc.classdetails_name_var,cc.classdetail_id_int,ccs.classsection_name_var, ccs.classsection_id_int,csc.student_imgurl_var,csc.`student_id_int`,CASE WHEN chs.housename IS NULL THEN 'No House' ELSE chs.housename END housename FROM campus_election_nomiation_registration cenr LEFT JOIN campus_student cs ON cs.student_admissionno_var = cenr.admissionNo LEFT JOIN campus_student_classdetails csc ON cs.student_id_int = csc.student_id_int LEFT JOIN campus_classdetail cc ON (csc.classdetail_id_int=cc.classdetail_id_int AND csc.locationId=cc.locationId) LEFT JOIN campus_classsection ccs ON csc.classsection_id_int=ccs.classsection_id_int LEFT JOIN campus_house_settings chs ON csc.student_house WHERE electionCategoryId=? AND isApproved='APPROVED' AND csc.student_house=? AND cs.student_gender_var IN(SELECT student_gender_var FROM campus_student WHERE student_id_int=?) ORDER BY studentName");
					ps_cadidateList.setString(1, rs_categoryList.getString("electionCategoryId"));
					ps_cadidateList.setString(2, activationFor.split(",")[2]);
		...
		try {
			...
			if (ps_categoryList != null && (!ps_categoryList.isClosed()) && ps_cadidateList!=null && ps_cadidateList.isClosed()) {
...
public String voteCount(String localIp, String admissionNo, String count2) {
	Connection conn =null;
	PreparedStatement psmt =null;
	ResultSet rs =null;
	PreparedStatement psmt1 =null;
	...
	int count1=0;
 	try{
		...
		psmt1=conn.prepareStatement("UPDATE campus_election_polling_machine_setting SET status='P' WHERE systemIp_var=? AND status='Y'");
...
public ArrayList<ElectionVo> getCategoryList(String classId, String accyear, String group, String titleID) {
	...
	ArrayList<ElectionVo> list =new ArrayList<ElectionVo>();
 	try{
			conn=JDBCConnection.getSeparateConnection();
			pstmt = conn.prepareStatement("SELECT * FROM campus_election_category_setting WHERE classId REGEXP CONCAT('(^|,)(', REPLACE(?, ',', '|'), ')(,|$)') AND accyearID=? AND groupID=? AND electionSettingID=? ");
...
public ArrayList<ElectionVo> getWinnerList(String accyear, String group,
	...
	try{
			conn=JDBCConnection.getSeparateConnection();
			pstmt1=conn.prepareStatement("SELECT electionCategoryId FROM campus_election_category_setting WHERE accyearID =? AND groupID =? AND electionSettingID=?");
...
public List<StudentRegistrationVo> studentSearchbyadmissionNoForApproval(
	...
	try {
		...
		conn = JDBCConnection.getSeparateConnection();
		pstmObj = conn
				.prepareStatement("SELECT student_admissionno_var FROM campus_student  WHERE student_admissionno_var LIKE ?");
