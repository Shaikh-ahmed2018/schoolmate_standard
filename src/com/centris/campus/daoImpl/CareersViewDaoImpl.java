...
public class CareersViewDaoImpl implements CareersViewDao {
	...
	public synchronized List<CareersViewVo> getAllCareersView(UserLoggingsPojo userLoggingsVo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
	...
	public synchronized String addJobs(CareersViewPojo pojo) {
		...
		try {
			...
			if ("NULL".equalsIgnoreCase(pojo.getCheck())) {
				 ...
				 * if (count == 0) {
				 */
				pstmt = conn.prepareStatement(InternalJobPostSQLUtilConstants.ADD_JOB_DETAILS);
				pstmt.setString(1, pojo.getJobcode());
				...
				count = pstmt.executeUpdate();
				if (count > 0) {
					//HelperClass.recordLog_Activity(pojo.getLog_audit_session(),"Settings","InternalJobPosting","Insert",pstmt.toString());
				...
				if ("deactive".equalsIgnoreCase(pojo.getStatus())) {
					...
					conn = JDBCConnection.getSeparateConnection();
					pstmt = conn.prepareStatement(InternalJobPostSQLUtilConstants.JOB_CHECK_APPLIEDJOBS);
					pstmt.setString(1, pojo.getJobcode());
					rs = pstmt.executeQuery();
					while (rs.next()) {
						count1 = rs.getInt("job");
					...
					if (count1 == 0) {
						...
						count = pstmt.executeUpdate();
						if (count > 0) {
							//HelperClass.recordLog_Activity(pojo.getLog_audit_session(),"Settings","InternalJobPosting","Update",pstmt.toString());
							status = MessageConstants.SUCCESS;
						} else {
							status = MessageConstants.FAILD;
						}
	...
	public synchronized List<CareersViewVo> searchDetails(String name,UserLoggingsPojo userLoggingsVo) {
