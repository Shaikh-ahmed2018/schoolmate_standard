...
//				vo.setStatus(rs.getString("status"));
	...
	public String UpdatingFirstLevelAdmissionApproval(
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection();
			pstmt = conn.prepareStatement(SQLUtilConstants.UPDATING_STATUS_DETAILS_FOR_TEMPORARY_REGISTRATION);
	...
	@Override
	public String InsertTemporaryStudentRegistration(ParentRequiresAppointmentForm secform,String log_audit_session,UserLoggingsPojo userLoggingsVo) {
		...
		try {
			...
			//pstmt = conn.prepareStatement(SQLUtilConstants.INSERTING_OFFLINE_REGISTRATION_FORMS);
			//temporary_admission_id,studentfirstName,studentlastname,dateofBirthId,gender,nationality,placeofbirth,state,religion,thersreligion,caste,othercaste,castecategory,othercastecategory,mothertongue,aadharNo,permanentaddress,addressofcommunication,preferedphno,classname,issibilings,siblingid,siblingname,isaluminiparents,isaluminifathername,fatheraluminiyear,isaluminimothername,motheraluminiyear,fathername,fathermobileno,fatheroccupation,fathermonthincome,fatheremailid,fatherofficialaddress,mothername,mothermobile,motheroccupation,mothermothlyincome,motheremailid,motherofficialaddress,createdTime,imageUrl,enquiryid,stream
			System.out.println("enquiry id is "+secform.getEnquiryid());
			if(secform.getEnquiryid() != null){
				pstmt = conn.prepareStatement(SQLUtilConstants.INSERTING_FIRST_REGISTRATION_FORMS);
				...
				pstmt.setString(20, secform.getClassname());
				pstmt.setString(21, secform.getIsSibling());
				pstmt.setString(22, secform.getIsSiblingId());
				pstmt.setString(23, secform.getIsSiblingName());
				pstmt.setString(24, secform.getParentsAlumni());
				pstmt.setString(25, secform.getFatherAlumniname());
				pstmt.setString(26, secform.getFatherAlumniyear());
				pstmt.setString(27, secform.getMotherAlumniname());
				pstmt.setString(28, secform.getMotherAlumniyear());
                pstmt.setString(29, secform.getFatherName());
				pstmt.setString(30, secform.getFatherMobileNo());
				...
				pstmt.setString(39, secform.getMotherOccupation());
				
				pstmt.setString(40, secform.getMotherDesignation());
				pstmt.setString(41, secform.getMotherDepartment());
				
				pstmt.setDouble(42, secform.getMotherMonthlyIncome());
				pstmt.setString(43, secform.getMotherEmail());
				pstmt.setString(44, secform.getMothersOfficialAddress());
                pstmt.setTimestamp(45, HelperClass.getCurrentTimestamp());
				pstmt.setString(46, secform.getImageString());
				pstmt.setString(47, secform.getEnquiryid().trim());
				pstmt.setString(48, secform.getStream());
				pstmt.setString(50, secform.getLocId());
				pstmt.setString(49, secform.getBirthCertificateFileString());
				pstmt.setString(51, secform.getAccyear());

				System.out.println("INSERTING_FIRST_REGISTRATION_FORMS-->>"+pstmt);

				i=pstmt.executeUpdate();
				if(i>0){
					HelperClass.recordLog_Activity(log_audit_session,"Admission","ParentFormSubmit","Insert",pstmt.toString(),userLoggingsVo);
				}
				...
				if(updateCount>0){
					HelperClass.recordLog_Activity(log_audit_session,"Admission","ParentFormSubmit","Delete",pstmt3.toString(),userLoggingsVo);
				}
			}
	...
	@Override
	public List<ParentRequiresAppointmentVO> searchadmissionsList(String searchName,UserLoggingsPojo userLoggingsVo) {
	...
	public ParentRequiresAppointmentVO EditingForConfirmingAdmissionDetails(
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection();
			pstmt = conn.prepareStatement(SQLUtilConstants.GET_SINGLE_FINAL_ADMISSION_DETAILS_LIST);
	...
	public String UpdatingFinalApprovalAdmissionStatus(
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement(SQLUtilConstants.FINAL_FETCHING_DETAILS_FROM_TABLE);
		...
		
			{
				 ...
				 application_recieved_by=rs.getString("application_recieved_by");
						...
						while(rs.next())
						{
						 ...
						 id = IDGenerator.getPrimaryKeyID("campus_student",userLoggingsVo);
							...
							PreparedStatement preParentReg = (PreparedStatement) JDBCConnection.getStatement(StudentRegistrationSQLUtilConstants.PARENT_REG,details.getCustid());
							...
							if (relationship_with_person.equals("father")) {
								...
								statement1.setString(1, IDGenerator.getPrimaryKeyID("campus_user",userLoggingsVo));
	...
	public String Insertthirdadmissionformat(
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement(SQLUtilConstants.INSERT_THIRD_ADMISSION_FORMAT);
  
			pstmt.setString(1, IDGenerator.getPrimaryKeyID("campus_third_admission_details",userLoggingsVo));
			pstmt.setString(2, parentform.getTemporary_studentfirstName());
			
			
			if(parentform.getGender() != null && !parentform.getGender().equalsIgnoreCase("")){
				pstmt.setString(3, parentform.getGender());
			}else{
				pstmt.setString(3, "");
			}
			
			pstmt.setString(4, HelperClass.convertUIToDatabase(parentform.getDateofBirthId()));
			if(parentform.getBelongsto() != null && !parentform.getBelongsto().equalsIgnoreCase("")){
				pstmt.setString(5, parentform.getBelongsto());
			}else{
				pstmt.setString(5, "");
			}
			pstmt.setString(6, parentform.getAadharno());
			
			pstmt.setString(7, parentform.getPer_address());
			pstmt.setString(8, parentform.getPre_address());
			pstmt.setString(9, parentform.getMothertongue());
			
			pstmt.setString(10, parentform.getBirth_place());
			pstmt.setString(11, parentform.getState());
			pstmt.setString(12, parentform.getNationality());
			pstmt.setString(13, parentform.getReligion());
			pstmt.setString(14, parentform.getCaste());
			pstmt.setString(15, parentform.getTemporary_fathername());
			pstmt.setString(16, parentform.getMothername());
			pstmt.setString(17, parentform.getFoccupation());
			pstmt.setString(18, parentform.getMoccupation());
			pstmt.setString(19, parentform.getFofficialaddress());
			pstmt.setString(20, parentform.getMofficialaddress());
			if(parentform.getBelongsto() != null && !parentform.getBelongsto().equalsIgnoreCase("")){
				pstmt.setString(21, parentform.getFincome());
			}else{
				pstmt.setString(21, "");
			}
			
			pstmt.setString(22, parentform.getMincome());
			pstmt.setString(23, parentform.getTemporary_fathermobile());
			pstmt.setString(24, parentform.getMothermobile());
			pstmt.setString(25, parentform.getQ_name());
			pstmt.setString(26, parentform.getBoard());
			pstmt.setString(27, parentform.getTemporary_school_name());
			pstmt.setString(28, parentform.getEnglish());
			pstmt.setString(29, parentform.getMathematics());
			pstmt.setString(30, parentform.getScience());
			pstmt.setString(31, parentform.getSocial());
			pstmt.setString(32, parentform.getSecond_lang());
			pstmt.setString(33, parentform.getSubject_one());
			pstmt.setString(34, parentform.getSubject_two());
			pstmt.setString(35, parentform.getSubject_three());
			pstmt.setString(36, parentform.getSubject_four());
			pstmt.setString(37, parentform.getSubject_five());
			pstmt.setString(38, parentform.getTotal_marks());
			pstmt.setString(39, parentform.getOpt_subone());
			pstmt.setString(40, parentform.getOpt_subtwo());
			pstmt.setString(41, parentform.getOpt_subthree());
			pstmt.setString(42, parentform.getOpt_subfour());
			if(parentform.getImageString() != null && !parentform.getImageString().equalsIgnoreCase("")){
				pstmt.setString(43, parentform.getImageString());
			}else{
				pstmt.setString(43, "");
			}
			pstmt.setString(44,
					IDGenerator.getAdmId("campus_third_admission_details",parentform.getCustid()));
			...
			if (rs > 0) {
				...
				pstmt1 = conn.prepareStatement("select stu_id from campus_third_admission_details order by createdDate desc limit 1");
				rs1=pstmt1.executeQuery();
				while(rs1.next()){
					result=rs1.getString("stu_id");
				}
	...
	public List<ParentRequiresAppointmentVO> getAdmissionRegDetails(ParentRequiresAppointmentVO appointmentVo,UserLoggingsPojo userLoggingsVo) {
		...
		try {
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pst = conn.prepareStatement(SQLUtilConstants.GET_ADMISSION_DETAILS);
	...
	public ParentRequiresAppointmentVO getSchoolDetails(String Enquiryid,UserLoggingsPojo userLoggingsVo) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in Inventory_DAOIMPL: getSchoolDetails : Starting");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ParentRequiresAppointmentVO vo = new ParentRequiresAppointmentVO();
		Connection conn = null;
		...
		try {
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement(SQLUtilConstants.GET_SCHOOL_DETAILS_BY_PARENT_FORM_PRINT);
			pstmt.setString(1, Enquiryid);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				vo.setLocId(rs.getString("Location_Id"));
				vo.setMobile_number(rs.getString("Location_Phone"));
				vo.setEmailId(rs.getString("emailId"));
				vo.setAddress(rs.getString("Location_Address"));
				vo.setSchool_name(rs.getString("Location_Name"));
			}
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in Inventory_DAOIMPL: getSchoolDetails : Ending");
		return vo;

	}

	public ParentRequiresAppointmentVO getStudentPreferedClass(ParentRequiresAppointmentVO appointmentVo,UserLoggingsPojo pojo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in Inventory_DAOIMPL: getStudentPreferedClass : Starting");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ParentRequiresAppointmentVO vo = new ParentRequiresAppointmentVO();
		Connection conn = null;
		...
		try {
			...
			pstmt = conn.prepareStatement("SELECT cped.class_id,cl.Location_Name FROM campus_parent_enquiry_details cped JOIN campus_location cl ON cl.Location_Id=cped.loc_id WHERE enquiry_id=?");
			pstmt.setString(1, appointmentVo.getEnquiryId());
			System.out.println("getStudentPreferedClass DaoImpl-->>"+pstmt);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				vo.setClassname(rs.getString("class_id"));
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in Inventory_DAOIMPL: getStudentPreferedClass : Ending");
		return vo;
	}

	public ParentRequiresAppointmentVO getStudentPreferedSchool(ParentRequiresAppointmentVO appointmentVo,UserLoggingsPojo pojo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in Inventory_DAOIMPL: getStudentPreferedSchool : Starting");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ParentRequiresAppointmentVO vo = new ParentRequiresAppointmentVO();
		Connection conn = null;
		...
		try {
			conn = JDBCConnection.getSeparateConnection(pojo);
			pstmt = conn.prepareStatement("SELECT cpeq.accyearId,cpeq.loc_id,cpeq.stu_parrelationship,cpeq.parents_name FROM campus_parent_enquiry_details cpeq JOIN campus_user cu ON  cu.employeecode=cpeq.enquiry_id WHERE cu.employeecode=?");
			pstmt.setString(1, appointmentVo.getEnquiryId());
			System.out.println("getStudentPreferedSchool DaoImpl-->>"+pstmt);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				vo.setAccyear(rs.getString("accyearId"));
				vo.setLocationName(rs.getString("loc_id"));
				if(rs.getString("stu_parrelationship").equalsIgnoreCase("father")) {
					vo.setFatherName(rs.getString("parents_name"));
				}
				else if(rs.getString("stu_parrelationship").equalsIgnoreCase("mother"))
				{
					vo.setMotherName(rs.getString("parents_name"));
				}
			}
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in Inventory_DAOIMPL: getStudentPreferedSchool : Ending");
		return vo;
	}
