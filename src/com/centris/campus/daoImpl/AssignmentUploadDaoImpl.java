...
public class AssignmentUploadDaoImpl implements AssignmentUploadDao{
	 ...
	 @Override
	 	public String insertAssignment(AssignmentViewVO view,UserLoggingsPojo userLoggingsVo) {
			...
			try {
				...
				String assignmentID=IDGenerator.getPrimaryKeyID("campus_assignment",userLoggingsVo);
				...
				if(count>0){
					HelperClass.recordLog_Activity(view.getLog_audit_session(),"Staff","Upload Assignment","Insert",ps_getstudents.toString(),userLoggingsVo);
	...
	@Override
		public ArrayList<AssignmentUploadVo> getStudentDetails(String section) {
			...
			try {
				...
				try {
					if(rs_getstudents!=null && !rs_getstudents.isClosed()){
						rs_getstudents.close();
					}
					if (ps_getstudents != null&& (!ps_getstudents.isClosed())) {
						ps_getstudents.close();
					}
		...
		public String updateAssignmentDetails(AssignmentUploadVo assignmentVo,UserLoggingsPojo userLoggingsVo) {
				...
				try {
					...
					if(count>0){
						HelperClass.recordLog_Activity(assignmentVo.getLog_audit_session(),"Staff","Upload Assignment","Update",ps_getstudents.toString(),userLoggingsVo);
						status="true";
					}else{
						status="false";
					}
		...
		@Override
		public String deleteAssignment(AssignmentViewVO view,UserLoggingsPojo userLoggingsVo) {
				...
				if(count>0){
					HelperClass.recordLog_Activity(view.getLog_audit_session(),"Staff","Upload Assignment","Delete",ps_deleteassgnment.toString(),userLoggingsVo);
		...
		public String addProject(ProjectVO viewVo,UserLoggingsPojo userLoggingsVo) {
				...
				try {
					...
					conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
					String projectID=IDGenerator.getPrimaryKeyID("campus_project",userLoggingsVo);
					...
					if(count>0){
						...
						HelperClass.recordLog_Activity(viewVo.getLog_audit_session(),"Staff","Project","Insert",pstmt.toString(),userLoggingsVo);
					...
					if(Count1>0){
						HelperClass.recordLog_Activity(viewVo.getLog_audit_session(),"Staff","Project","Insert",pstmt1.toString(),userLoggingsVo);
						status="true";
					}else{
		...
		public ArrayList<ProjectVO> getProjectList(String userId, String accYearString, String searchTerm) {
			...
			try {
				...
				while(rs.next()){
					...
					vo.setAssigneddate(HelperClass.convertDatabaseToUI(rs.getString("AssignedDate")));
					vo.setSubmissiondate(HelperClass.convertDatabaseToUI(rs.getString("SubmissionDate")));
		...
		public ProjectVO getSingleProject(String projectId) {
			...
			try {
				...
				try {
					...
					if(ps_getproject1!=null && !ps_getproject1.isClosed()){
						ps_getproject1.close();
					}
					if(rs_project != null && (!rs_project.isClosed()))
					{
						rs_project.close();
		...
		public String updateProjectDetails(ProjectVO projectvo,UserLoggingsPojo userLoggingsVo) {
			...
			try {
				...
				if(count>0){
					HelperClass.recordLog_Activity(projectvo.getLog_audit_session(),"Staff","Project","Update",ps_getstudents.toString(),userLoggingsVo);
					status="true";
				}else{
					status="false";
				}
		...
		public String deleteProject(ProjectVO project,UserLoggingsPojo userLoggingsVo) {
			...
			if(count>0){
				...
				HelperClass.recordLog_Activity(project.getLog_audit_session(),"Staff","Project","Delete",ps_deleteproject.toString(),userLoggingsVo);
				...
				if(count>0){
					HelperClass.recordLog_Activity(project.getLog_audit_session(),"Staff","Project","Delete",ps_deleteprojectdetails.toString(),userLoggingsVo);
					status="true";
				}else{
					status="false";
				}
