...
public  class NewRegistrationDaoImpl implements NewRegistrationDao {
	...
	public void getregdetails(NewRegistrationForm newregform) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in NewRegistrationDaoImpl:getregdetails:Ending");
	}
...

	{
		...
		try {
			...
			String id1 = IDGenerator.getPrimaryKeyID("campus_parent_enquiry_details",userLoggingsVo);
			...
			pstmt.setString(12,registrationform.getLocation_id());
			pstmt.setString(13,registrationform.getAccyear());
			System.out.println("REGISTER_PARENT_ENQUIRY_DETAILS:;:"+pstmt);
			result1 = pstmt.executeUpdate();

			if (result1 > 0 ) {
				HelperClass.recordLog_Activity(registrationform.getLog_audit_session(),"Admission","Issued","Insert",pstmt.toString(),userLoggingsVo);
				status = "true";
			} else {
				status = "false";
			}
	...
	public String saveparentsubmittingdetailstoschool(
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in NewRegistrationDaoImpl:saveparentsubmittingdetailstoschool:Starting");
		...
		try {
			String id1 = IDGenerator.getPrimaryKeyID("campus_temporary_admisssion_details",userLoggingsVo);
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in NewRegistrationDaoImpl:saveparentsubmittingdetailstoschool:Ending");
		return status;
	}
