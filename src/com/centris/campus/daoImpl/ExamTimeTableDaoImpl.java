...
public class ExamTimeTableDaoImpl implements ExamTimeTableDao {
	...
	public ArrayList<ExamTimetablePojo> getExamTimeTableDetails(String classId,
		...
		try {
			...
			subject_pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.GET_ALL_SUBJECTS);
			...
			while (subject_pstmt_rs.next()) {
				...
				if (checkExamDate(classId, subjectId, exam,userLoggingsVo)) {
	...
	public boolean checkExamDate(String classid, String subjectid, String examid, UserLoggingsPojo userLoggingsVo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
	...
	public ArrayList<ExamTimetablePojo> getExamdetails(UserLoggingsPojo userLoggingsVo) {
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.GET_EXAMDETAILS);
	...
	public ArrayList<ExamTimetablePojo> getclassdetails(UserLoggingsPojo userLoggingsVo) {
		...
		try {
			...
			pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.GET_CLASSDETAILS);
...
//For inserting New Exam
	public String insertExam(ExamDetailsForm addExam) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: insertExam  Starting");
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String msg="";
		...
		try {
		    ...
		    primaryid = IDGenerator.getPrimaryKeyID("campus_examination",addExam.getCustid());
		     
            conn = JDBCConnection.getSeparateConnection(addExam.getCustid());
			...
			if(i>0)
			{
				HelperClass.recordLog_Activity(addExam.getLog_audit_session(),"Exam","Exam Setting","insertExam",pstmt.toString(),addExam.getCustid());
				msg="true";
			}
			else
			{
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: insertExam  Ending");
		return msg;
	}
...
//For Listing the Exams
	public ArrayList<ExaminationDetailsVo> getexamsettingsDetails(
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: getexamsettingsDetails  Starting");
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int sno = 0;
		...
		try{
	         ...
	         System.out.println("EXAM DETAILS "+pstmt);
	         rs= pstmt.executeQuery();
	         while(rs.next()){
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: getexamsettingsDetails  Ending");
		return list;
	}
//For Deleting the Exam Settings
		...
		logger.info(JDate.getTimeString(new Date())
				...
				try{
					conn = JDBCConnection.getSeparateConnection(addExam.getCustid());
			         ...
			         if(i>0)
			         {
			        	 HelperClass.recordLog_Activity(addExam.getLog_audit_session(),"Exam","Exam Setting","Delete",pstmt.toString(),addExam.getCustid());
			        	msg="true";
			         }
			         else
			         {
	...
	//For Edit the Exam Details
     public String editexamsettings(String editid,ExamDetailsForm editExam) {
		System.out.println("deleteid from daoimpl:" +editid);

		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: editexamsettings Starting");
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String msg=null;
		try{
			conn = JDBCConnection.getSeparateConnection(editExam.getCustid());
			...
			if(i>0)
			{
				 HelperClass.recordLog_Activity(editExam.getLog_audit_session(),"Exam","Exam Setting","Update",pstmt.toString(),editExam.getCustid());
				msg="true";
			}
			else
			{
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: editexamsettings Ending");
		return msg;
}
	...
	public ArrayList<ExaminationDetailsVo> getstatusexamsettingsDetails(String accyear, String schoolLocation,UserLoggingsPojo custdetails) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: getstatusexamsettingsDetails Starting");
		Connection conn = null;
		PreparedStatement pstmt = null;
		PreparedStatement pstmt1=null;
		ResultSet rs = null;
		...
		try { 
			conn = JDBCConnection.getSeparateConnection(custdetails);
			String query="SELECT classid,examid,examcode,examname,startdate,enddate,isapplicableperiodic FROM campus_examination";
			...
			while (rs.next()) {
				...
				pstmt1 = conn.prepareStatement("SELECT COUNT(*) AS studentcount FROM `campus_student` st JOIN `campus_student_classdetails` csc ON csc.`student_id_int`=st.`student_id_int` WHERE csc.`fms_acadamicyear_id_int`=? AND csc.`locationId`=? AND csc.`classdetail_id_int`=? AND st.`student_status_var`='active'");
				...
				pstmt1.setString(2, schoolLocation);
				pstmt1.setString(3, rs.getString("classid"));
				rs1 = pstmt1.executeQuery();
				while(rs1.next()) {
				...
				if(clascount>0){
					...
					PreparedStatement dispstmt = conn.prepareStatement("select subjectID from campus_subject where classid=? AND isActive='Y'");
					//dispstmt.setString(1,schoolLocation);
					//dispstmt.setString(2,accyear);
					dispstmt.setString(1,rs.getString("classid"));
					...
					while(disrs.next()){
						...
						PreparedStatement exampstmt =  conn.prepareStatement("SELECT COUNT(`stu_id`) FROM `campus_studentwise_mark_details` WHERE `classid` = ? AND `Academic_yearid` = ? AND `location_id` = ? AND `exam_id`=? AND sub_id=?");
						exampstmt.setString(1,rs.getString("classid"));
						exampstmt.setString(2,accyear);
						exampstmt.setString(3,schoolLocation);
						exampstmt.setString(4,rs.getString("examid"));
						exampstmt.setString(5,disrs.getString("subjectID"));
						ResultSet examrs = exampstmt.executeQuery();
						while(examrs.next()){
							examcount = examcount+examrs.getInt(1);
							if(examcount == 0){
								count = 1;
							}
						}
					...
					disrs.close();
					dispstmt.close();
					if(examcount/classexamcount==clascount){
	...
	public String saveExaminationClassMapping(ArrayList<ExamTimetablePojo> examinationclassmappinglist
		...
		try {
			...
			for (int i = 0; i < examinationclassmappinglist.size(); i++) {
				...
				if (checkExamDate(examinationclassmappinglist.get(i).getClassId(), examinationclassmappinglist.get(i)
						.getSubId(), examinationclassmappinglist.get(i).getExamId(),userLoggingsVo)) {

					// UPDATE_EXAMDATE

					pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.UPDATE_EXAMDATE);
					pstmt.setString(1, examinationclassmappinglist.get(i).getExamDate());
					pstmt.setString(2, examinationclassmappinglist.get(i).getExamStartTime());
					pstmt.setString(3, examinationclassmappinglist.get(i).getExamEndTime());
					pstmt.setString(4, examinationclassmappinglist.get(i).getCreatedBy());

					pstmt.setString(5, examinationclassmappinglist.get(i).getClassId());
					pstmt.setString(6, examinationclassmappinglist.get(i).getSubId());
					pstmt.setString(7, examinationclassmappinglist.get(i).getExamId());
					
					count = pstmt.executeUpdate();
					
					if(count>0){
						//System.out.println("UPDATE_EXAMDATE -->>"+pstmt);
						HelperClass.recordLog_Activity(log_audit_session,"Exam","ExamTimetable","Update",pstmt.toString(),userLoggingsVo);
					}

				} else {

					pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.STORE_EXAM_TIMETABLE_DETAILS);

					pstmt.setString(1, examinationclassmappinglist.get(i).getClassId());
					pstmt.setString(2, examinationclassmappinglist.get(i).getSubId());
					pstmt.setString(3, examinationclassmappinglist.get(i).getExamId());
					pstmt.setString(4, examinationclassmappinglist.get(i).getExamDate());
					pstmt.setString(5, examinationclassmappinglist.get(i).getExamStartTime());
					pstmt.setString(6, examinationclassmappinglist.get(i).getExamEndTime());
					pstmt.setString(7, examinationclassmappinglist.get(i).getCreatedBy());
                    
					count = pstmt.executeUpdate();
					
					if(count>0){
						//System.out.println("STORE_EXAM_TIMETABLE_DETAILS -->>"+pstmt);
						HelperClass.recordLog_Activity(log_audit_session,"Exam","ExamTimetable","Insert",pstmt.toString(),userLoggingsVo);
					}
				}
	...
	public ArrayList<ExaminationDetailsVo> getEaxmMarksListYear(String accyear, String locid,UserLoggingsPojo userLoggingsVo) {
		...
		try {
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pstmt = conn.prepareStatement("select acadamic_id,acadamic_year,startDate,endDate,l.Location_Id,l.Location_Name from campus_acadamicyear,campus_location l where acadamic_id like ? and l.Location_Id like ? and l.isActive='Y' order by startDate,Location_Name");
			pstmt.setString(1,accyear);
			...
			/*	
				 pstmtexm = conn.prepareStatement(ExamTimeTableSqlUtils.GETEXAMSETTINGSDETAILS);
				 pstmtexm.setString(1, rs.getString("acadamic_id"));
				 pstmtexm.setString(2, rs.getString("Location_Id"));
					rsexm = pstmtexm.executeQuery();
					while (rsexm.next()) {*/
						//yearcount++;
				//SELECT DISTINCT `examid`,`classid` FROM `campus_examination` WHERE `acadamicyear`='ACY2' AND `loc_id`='LOC1'
			
				//pstmt1 = conn.prepareStatement("SELECT COUNT(*) AS studentcount FROM `campus_student` st JOIN `campus_student_classdetails` csc ON csc.`student_id_int`=st.`student_id_int` WHERE csc.`fms_acadamicyear_id_int`=? AND csc.`locationId`=? AND st.`student_status_var`='active'");
				int total=0;
				int classcount=0;
				int stcount=0;
				
						pstmt1 = conn.prepareStatement("SELECT DISTINCT e.`examid`,e.`classid` FROM `campus_examination` e JOIN `campus_classdetail` cc ON cc.`classdetail_id_int`=e.`classid` WHERE `acadamicyear`=? AND `loc_id`=? AND cc.`isActive`='Y'");
						pstmt1.setString(1,  rs.getString("acadamic_id"));
						pstmt1.setString(2, rs.getString("Location_Id"));
						rs1 = pstmt1.executeQuery();
						while(rs1.next()) {
							pstmt2 = conn.prepareStatement("SELECT COUNT(*) FROM `campus_student` st JOIN `campus_student_classdetails` ccd ON st.`student_id_int`=ccd.`student_id_int` WHERE ccd.`fms_acadamicyear_id_int`=? AND ccd.`locationId`=? AND ccd.`classdetail_id_int`=? AND st.`student_status_var`='active'");
							pstmt2.setString(1,  rs.getString("acadamic_id"));
							pstmt2.setString(2, rs.getString("Location_Id"));
							pstmt2.setString(3, rs1.getString("classid"));
							rs2= pstmt2.executeQuery();
							while(rs2.next()) {
								stcount=rs2.getInt(1);
							}
						...
						//	classcnt.setString(3, rs.getString("examid"));
							...
							total=total+classcount*stcount;
							System.out.println(total+" "+classcount+"*"+stcount);
						}
						//if(stcount>0){
					//	PreparedStatement dispstmt = conn.prepareStatement("SELECT `subjectID` FROM `campus_subject` WHERE `locationId`=?");
						/*PreparedStatement dispstmt = conn.prepareStatement("SELECT `examid`,`classid` FROM `campus_examination` WHERE `acadamicyear`=? AND `loc_id`=?");
						dispstmt.setString(1,rs.getString("acadamic_id"));
						dispstmt.setString(2,rs.getString("Location_Id"));
						ResultSet disrs = dispstmt.executeQuery();
						while(disrs.next()){*/
							//	PreparedStatement exampstmt =  conn.prepareStatement("SELECT COUNT(`stu_id`) FROM `campus_studentwise_mark_details` WHERE `Academic_yearid` = ? AND `location_id` = ? AND `exam_id`=? AND sub_id=?");
						    /*	PreparedStatement exampstmt =  conn.prepareStatement("SELECT COUNT(*) FROM `campus_studentwise_mark_details` WHERE `location_id`=? AND `Academic_yearid`=?");
								exampstmt.setString(1, disrs.getString("classid"));
								exampstmt.setString(2,disrs.getString("examid"));
						    	exampstmt.setString(1,rs.getString("Location_Id"));
								exampstmt.setString(2,rs.getString("acadamic_id"));
								System.out.println(exampstmt);
								ResultSet examrs = exampstmt.executeQuery();
								while(examrs.next()){
									examcount = examcount+examrs.getInt(1);
									if(examcount == 0){
					...
					/*}else{
						...
						PreparedStatement exampstmt =  conn.prepareStatement("SELECT COUNT(csm.`stu_id`) FROM `campus_studentwise_mark_details` csm JOIN `campus_student` cs ON cs.`student_id_int`=csm.`stu_id` WHERE csm.`Academic_yearid` = ? AND csm.`location_id` = ? AND cs.`student_status_var`='active'");
						exampstmt.setString(1,rs.getString("acadamic_id"));
						exampstmt.setString(2,rs.getString("Location_Id"));
						//exampstmt.setString(3,rs1.getString("examid"));
						ResultSet examrs = exampstmt.executeQuery();
						while(examrs.next()){
							examcount = examcount+examrs.getInt(1);
						}
						...
						System.out.println(total+" "+examcount);
					if(total==examcount){
						exampojo.setStatus("Set");
					}else{
						exampojo.setStatus("Not Set");
		...
		try {
			...
			if(validation==true){
				for(int i=0; i<examcode.length; i++){
					...
					if(a>0){
						//HelperClass.recordLog_Activity(log_audit_session,"Exam","GradeDependency","Insert",pstmt.toString());
						result="success";
					}else{
						result="failure";
					}
		...
		ResultSet rs=null;
			...
			try{
				...
				if(count1==0){
					...
					count =pstmt.executeUpdate();
					if(count>0){
						//HelperClass.recordLog_Activity(log_audit_session,"Exam","Grade Dependency","Insert",pstmt.toString());
						status="success";
					}else{
						status="unsuccess";
						}
	...
	public ArrayList<ExaminationDetailsVo> getexamsettingsDetailsonClass(String accyearid, String location,
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: getexamsettingsDetailsonClass  Starting");
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int sno = 0;
		...
		try{
			 ...
			 if(classid.equalsIgnoreCase("%%")){
				 pstmt = conn.prepareStatement("SELECT exm.status_date,exm.`status_date`,exm.status,exm.classid,exm.`examid`,exm.`examcode`,exm.`examname`,exm.`startdate`,exm.`enddate`,exm.`isapplicableperiodic`,cls.`classdetails_name_var`,cet.`examtypename`,cet.`examtypeid` FROM campus_examination exm LEFT JOIN `campus_classdetail` cls ON cls.`classdetail_id_int`=exm.`classid` AND cls.`locationId`=exm.`loc_id` JOIN `campus_classstream` ccs ON ccs.`classstream_id_int`=cls.`classstream_id_int` LEFT JOIN `campus_examtype` cet ON cet.`examtypeid`=exm.`examtype` WHERE exm.`startdate` BETWEEN  DATE_FORMAT(NOW() ,'%Y-%m-01') AND LAST_DAY(NOW()) AND acadamicyear=? AND loc_id like ? AND exm.classid LIKE ? AND ccs.`isActive`='Y' AND cls.`isActive`='Y' ORDER BY LENGTH(cls.classdetail_id_int),cls.classdetail_id_int,exm.`startdate`");
				 pstmt.setString(3, classid);
			 }else{
				 pstmt = conn.prepareStatement("SELECT exm.status_date,exm.status,exm.classid,exm.`examid`,exm.`examcode`,exm.`examname`,exm.`startdate`,exm.`enddate`,exm.`isapplicableperiodic`,cls.`classdetails_name_var`,cet.`examtypename`,cet.`examtypeid` FROM campus_examination exm LEFT JOIN `campus_classdetail` cls ON cls.`classdetail_id_int`=exm.`classid` AND cls.`locationId`=exm.`loc_id` JOIN `campus_classstream` ccs ON ccs.`classstream_id_int`=cls.`classstream_id_int` LEFT JOIN `campus_examtype` cet ON cet.`examtypeid`=exm.`examtype` WHERE acadamicyear=? AND loc_id =? AND exm.classid = ? AND ccs.`isActive`='Y' AND cls.`isActive`='Y' ORDER BY LENGTH(cls.classdetail_id_int),cls.classdetail_id_int,exm.`startdate`");
				 pstmt.setString(3, classid);
			 }
	         ...
	         while(rs.next()){
	        	 ...
	        	 obj.setEndDate(HelperClass.convertDatabaseToUI(rs.getString("enddate")));
	        	 obj.setStatus(rs.getString("status"));
	        	 if(rs.getString("status_date")!=null){
	        		 obj.setStatus1(HelperClass.convertDatabaseToUI(rs.getString("status_date")));
	        	 }
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExamTimeTableDaoImpl: getexamsettingsDetailsonClass  Ending");
		return list;
	}
	@Override
	public ArrayList<ExaminationDetailsVo> getExamNameList(String locationid, String classid, String accyear,UserLoggingsPojo userLoggingsVo) {
	...
	public String resultDeclare(String accyear, String examcode, String location, UserLoggingsPojo custdetails,String msg) {
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(custdetails);
			pstmt = conn.prepareStatement("UPDATE `campus_examination` SET `status`=?,`status_date`=NOW() WHERE `examid`=? AND `acadamicyear`=? AND `loc_id`=?");
	...
	public String timetableset(String examid, String location, String accyear, UserLoggingsPojo custdetails) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExaminationTimeTableServiceIMPL: timetableset Starting");
		...
		try {
			conn = JDBCConnection.getSeparateConnection(custdetails);
			pstmt = conn.prepareStatement("SELECT COUNT(*) FROM `campus_exam_timetable` WHERE `examcode`=? AND `accyear_id`=? AND `loc_id`=?");
			pstmt.setString(1, examid);
			pstmt.setString(2, accyear);
			pstmt.setString(3, location);
			System.out.println("EXAMTIME TAble "+pstmt);
			rs=pstmt.executeQuery();
			//System.out.println("EXAMTIME TAble "+res);
			while(rs.next()){
				count=rs.getInt(1);
			}
			if(count>0){
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in  ExaminationTimeTableServiceIMPL: timetableset Ending");
		return status;

	}

	@Override
	public ArrayList<ExaminationDetailsVo> getexamDeclared(String academic_year, String location,
			UserLoggingsPojo custdetails, String classid) {
		...
		try{
			 conn = JDBCConnection.getSeparateConnection(custdetails);
	         //
				 pstmt = conn.prepareStatement("SELECT exm.status_date,exm.status,exm.classid,exm.`examid`,exm.`examcode`,exm.`examname`,exm.`startdate`,exm.`enddate`,exm.`isapplicableperiodic`,cls.`classdetails_name_var`,cet.`examtypename`,cet.`examtypeid` FROM campus_examination exm LEFT JOIN `campus_classdetail` cls ON cls.`classdetail_id_int`=exm.`classid` AND cls.`locationId`=exm.`loc_id` JOIN `campus_classstream` ccs ON ccs.`classstream_id_int`=cls.`classstream_id_int` LEFT JOIN `campus_examtype` cet ON cet.`examtypeid`=exm.`examtype` WHERE acadamicyear=? AND loc_id =? AND ccs.`isActive`='Y' AND cls.`isActive`='Y' AND exm.`status`='Declared' ORDER BY LENGTH(cls.classdetail_id_int),cls.classdetail_id_int,exm.`startdate`");
	...
	/*public ArrayList<ExaminationDetailsVo> getClassWiseExamListDetails(String accyear, String schoolLocation,UserLoggingsPojo custdetails) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
		...
		try { 
			conn = JDBCConnection.getSeparateConnection(custdetails);
			pstmt=conn.prepareStatement("SELECT exm.status,exm.classid,exm.examid,exm.examcode,exm.examname,exm.startdate,exm.enddate,exm.isapplicableperiodic,cls.classdetails_name_var,cet.examtypename,cet.examtypeid FROM campus_examination exm LEFT JOIN campus_classdetail cls ON cls.classdetail_id_int=exm.classid AND cls.locationId=exm.loc_id LEFT JOIN campus_classstream ccs ON ccs.classstream_id_int=cls.classstream_id_int LEFT JOIN campus_examtype cet ON cet.examtypeid=exm.examtype WHERE exm.acadamicyear=? AND exm.loc_id =? AND cls.isActive='Y' AND ccs.isActive='Y' ORDER BY LENGTH(cls.classdetail_id_int),cls.classdetail_id_int,exm.startdate");
			rs = pstmt.executeQuery();
			while (rs.next()) {
				System.out.println("loop");
				ExaminationDetailsVo exampojo = new ExaminationDetailsVo();
	...
	public ArrayList<ExaminationDetailsVo> getExamNameListByPeriodicExam(String locationid, String accyear, UserLoggingsPojo custdetails, String classid) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExaminationTimeTableServiceIMPL: getExamNameListByPeriodicExam Starting");
		ArrayList<ExaminationDetailsVo> examlist = new ArrayList<ExaminationDetailsVo>();
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(custdetails);
			pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.GET_PERIODICEXAM_DETAILS);
			pstmt.setString(1, locationid);
			pstmt.setString(2, classid);
			pstmt.setString(3, accyear);
			rs = pstmt.executeQuery();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in  ExaminationTimeTableServiceIMPL: getExamNameListByPeriodicExam Ending");
		return examlist;
	...
	public ArrayList<ExaminationDetailsVo> getHalflyExamNameList(String locationid, String accyear, UserLoggingsPojo custdetails, String classid) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExaminationTimeTableServiceIMPL: getHalflyExamNameList Starting");
		ArrayList<ExaminationDetailsVo> examlist = new ArrayList<ExaminationDetailsVo>();
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(custdetails);
			pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.GET_HALFYEARLYEXAM_DETAILS);
			pstmt.setString(1, locationid);
			pstmt.setString(2, classid);
			pstmt.setString(3, accyear);
			rs = pstmt.executeQuery();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in  ExaminationTimeTableServiceIMPL: getHalflyExamNameList Ending");
		return examlist;
	}
	...
	public ArrayList<ExaminationDetailsVo> getYearlyExamNameList(String locationid, String accyear, UserLoggingsPojo custdetails, String classid) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in ExaminationTimeTableServiceIMPL: getYearlyExamNameList Starting");
		ArrayList<ExaminationDetailsVo> examlist = new ArrayList<ExaminationDetailsVo>();
		...
		try {
			...
			conn = JDBCConnection.getSeparateConnection(custdetails);
			pstmt = conn.prepareStatement(ExamTimeTableSqlUtils.GET_YEARLYEXAM_DETAILS);
			pstmt.setString(1, locationid);
			pstmt.setString(2, classid);
			pstmt.setString(3, accyear);
			rs = pstmt.executeQuery();
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in  ExaminationTimeTableServiceIMPL: getYearlyExamNameList Ending");
		return examlist;
	}
}