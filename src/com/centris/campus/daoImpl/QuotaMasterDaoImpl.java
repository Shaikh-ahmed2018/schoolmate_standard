...
public class QuotaMasterDaoImpl implements QuotaMasterDao {
	...
	public synchronized ArrayList<QuotaMasterVO> getQuotaDetails(UserLoggingsPojo userLoggingsVo) {
	...
	public synchronized String updateQuotaDetails(QuotaMasterPojo pojo) {
		...
		try {
			...
			if (rs.getInt(1) == 0) {
				...
				pstmt = conn.prepareStatement(SQLUtilConstants.UPDATE_QUOTA_DETAILS);
				pstmt.setString(1, pojo.getQuotaName().trim());
	...
	public synchronized ArrayList<QuotaMasterVO> searchQuota(
			QuotaMasterVO searchvo,UserLoggingsPojo userLoggingsVo) {
