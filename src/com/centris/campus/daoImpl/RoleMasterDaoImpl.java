...
public class RoleMasterDaoImpl implements RoleMasterDao {
	...
	@Override
	public synchronized String addRole(RoleMasterPojo roleMasterPojo,UserLoggingsPojo userLoggingsVo)
			throws Exception {
		...
		try {
			...
			if (!(daoImpl.validateRoleName(roleMasterPojo.getRoleName(),userLoggingsVo))) {
				count = psrole.executeUpdate();
			}
			if (count > 0) {
				HelperClass.recordLog_Activity(roleMasterPojo.getLog_audit_session(),"Settings","RoleDetails","Insert",psrole.toString(),userLoggingsVo);
	...
	public String updateRole(RoleMasterPojo roleMasterPojo,UserLoggingsPojo userLoggingsVo) throws Exception {
		...
		try {
				...
				if (count > 0) {
					HelperClass.recordLog_Activity(roleMasterPojo.getLog_audit_session(),"Settings","RoleDetails","Update",psrole.toString(), userLoggingsVo);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in RoleMasterDaoImpl:updateRole Ending");

		return successMessage;
	}

	public synchronized List<RoleMasterPojo> getRoles(UserLoggingsPojo custdetails,String location_id) throws Exception {
	...
	@Override
	public synchronized String deleteRole(RoleMasterPojo roleMasterPojo,UserLoggingsPojo userLoggingsVo)
			throws Exception {
		...
		try {
			...
			if (result > 0) {
				...
				if (result > 0) {
					...
					successMessage = "Failed to delete Role Details";
				}
	...
	@Override
	public synchronized RoleMasterPojo updateRole1(String roleCodeId,UserLoggingsPojo userLoggingsVo)
			throws Exception {
		...
		try {
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			psgetrolesUpdate = conn.prepareStatement(RoleMasterSqlConstants.UPDATE_ROLES);
	...
	public synchronized boolean validateRoleName(String roleName, UserLoggingsPojo userLoggingsVo)
			throws Exception {
	...
	public synchronized boolean validateRoleNameUpdate(String roleName,String roleid,UserLoggingsPojo userLoggingsVo) throws Exception {
		...
		try {
			conn = JDBCConnection.getSeparateConnection(userLoggingsVo);
			pscheckRoleName = conn.prepareStatement(RoleMasterSqlConstants.VALIDATE_ROLE_NAME_UPDATE);
			pscheckRoleName.setString(1, roleName);
