...
public static String getPrimaryKeyID(String tableName) throws SQLException {
		...
		try {
			...
			else if(tableName.equalsIgnoreCase("log_audit"))
			{
				pre="SES";
			}
			
			else if (tableName.equalsIgnoreCase("campus_expenditure")) {
				pre = "EXP";
			}
			else if (tableName.equalsIgnoreCase("birthday_sms")) {
				pre = "BDY";
			}
...
/*public static String getPrimaryKeyID(String tableName,String custid) throws SQLException {
		...
		try {
			...
			int currentID = 0;
			String pre = null;
			String previousID = getPreviousID(tableName,custid);
			System.out.println("previousID "+previousID);
			if (previousID == null) {
				currentID = 0;
			} else {
			...
			else if (tableName.equalsIgnoreCase("campus_fee_stage_amount")) {
				pre = "FSA";
			}
			
			else if(tableName.equalsIgnoreCase("sms_any_other_sms"))
			{
				pre="OTS";
			}

			else if(tableName.equalsIgnoreCase("campus_syllabus_details"))
			{
				pre="CSD";
			}
...
public static String getPrimaryKeyID(String tableName,UserLoggingsPojo dbinfo) throws SQLException {
		...
		/*	System.out.println("tableName "+tableName);*/
			int currentID = 0;
			String pre = null;
			String previousID = getPreviousID(tableName,dbinfo);
			/*System.out.println("previousID "+previousID);*/
			if (previousID == null) {
				currentID = 0;
			} else {
			...
			else  if (tableName.equalsIgnoreCase("campus_library_other_subscriber")){ 
				pre ="LSD";
			}
			else  if (tableName.equalsIgnoreCase("campus_library_stock_entry")){ 
				pre ="LSE";
			...
			else if(tableName.equalsIgnoreCase("campus_bank")){
				pre = "BNK";
			}
			else if(tableName.equalsIgnoreCase("campus_bank_branch")){
				pre = "BRH";	
			}
			else if (tableName.equalsIgnoreCase("birthday_sms")) {
				pre = "BDY";
			...
			else if (tableName.equalsIgnoreCase("campus_sms_fee_students")) {
				pre = "CFS";
			}
			else if (tableName.equalsIgnoreCase("campus_exam_max_marks_setup")) {
				pre = "MMS";
			}
			else if (tableName.equalsIgnoreCase("campus_exam_report_setup")) {
				pre = "RTS";
			}
			{
	...
	private static String getPreviousID(String tableName,String custid) throws SQLException {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
		...
		try {
			...
			ps = (PreparedStatement) JDBCConnection.getStatement(sql,custid);
			...
			ps = (PreparedStatement) JDBCConnection.getStatement(sql2,custid);
			rs = ps.executeQuery();
			while (rs.next()) {
				counter++;
	...
	private static String getPreviousID(String tableName,UserLoggingsPojo dbinfo) throws SQLException {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
		...
		try {
			...
			ps = (PreparedStatement) JDBCConnection.getStatement(sql,dbinfo);
			rs = ps.executeQuery();
			while (rs.next()) {
				columnName = (String) rs.getString("Column_name");
				System.out.println("columnName"+columnName);
			...
			String sql2 = "select max(" + columnName + ") from " + tableName
					+ " group by length(" + columnName + ") desc limit 1";
			System.out.println(sql2);
			ps = (PreparedStatement) JDBCConnection.getStatement(sql2,dbinfo);
			rs = ps.executeQuery();
			while (rs.next()) {
				counter++;
				columnName = (String) rs.getString(1);
	...
	private static String getPreviousID(String tableName) throws SQLException {
		...
		try {
			...
			CustomerDBDetails dbdetails = null;
			String sql = "SHOW KEYS FROM " + tableName
					+ " WHERE Key_name = 'PRIMARY'";
...
//	}
	...
	public static String getAdmId(String tableName,String custid) throws SQLException {
		
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getAdmId Starting");
		String NextID = null;
		try {
			tableName = tableName.trim().toLowerCase();
			int currentID = 0;
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getAdmId Ending");
	...
	public List<PageFilterVo> getstaffIdCardDesignList(PageFilterpojo filterpojo, UserLoggingsPojo userLoggingsVo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffIdCardDesignList : Starting");
		...
		try{
			...
			psmt=conn.prepareStatement("SELECT caddr.address_line1,caddr.address_line2,caddr.tel_phone,csi.school_logo,csi.school_name,ca.acadamic_id,ca.acadamic_year,cl.Location_Id,cl.Location_Name,cl.Location_Address,cl.Location_phone,cd.DEPT_ID,cd.DEPT_NAME FROM campus_school_info csi,campus_acadamicyear ca,campus_department cd ,campus_location cl,campus_school_address caddr WHERE cl.loc_add_id=caddr.scl_address_id AND cl.Location_Id LIKE ? AND ca.acadamic_id LIKE ? AND cd.DEPT_ID LIKE ? and cl.isActive='Y' and ca.isActive='Y' and cd.isActive='Y' ORDER BY CAST(SUBSTR(cl.Location_Id,4)AS UNSIGNED),CAST(SUBSTR(cd.DEPT_ID,4)AS UNSIGNED)");
			...
			while(rs.next()){
				...
				filterVo.setLocationName(rs.getString("school_name"));
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffIdCardDesignList : Ending");
	...
	public List<PageFilterVo> getSingleStaffCardDesignList(
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getSingleStaffCardDesignList : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getSingleStaffCardDesignList : Ending");
	...
	public List<PageFilterVo> getstaffSingleIDCard(PageFilterpojo filterpojo) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSingleIDCard : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSingleIDCard : Ending");
	...
	public List<StudentRegistrationVo> getStudentListforPrint(
			...
			String sectionid, String streamId, String flag, UserLoggingsPojo userLoggingsVo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator : getStudentListforPrint  Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator : getStudentListforPrint  Ending");
	...
	public List<PageFilterVo> getstaffIdCardprintList(PageFilterpojo filterpojo) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffIdCardprintList : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffIdCardprintList : Ending");
	...
	public List<PageFilterVo> getstaffSearch(String searchTerm) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearch : Starting");
...
psmt2=conn.prepareStatement("SELECT DISTINCT ct.TeacherID,ct.FirstName,ct.department,cd.DEPT_NAME,ct.presentAddress,ct.designation,des.designationName,cay.acadamic_id,cay.acadamic_year,cl.Location_Id,cl.Location_Name,ct.isActive,ct.mobileNo FROM campus_acadamicyear cay,campus_teachers ct LEFT JOIN campus_department cd ON cd.DEPT_ID=ct.department LEFT JOIN campus_designation des ON des.DesignationCode=ct.designation LEFT JOIN campus_location cl ON ct.Loc_ID=cl.Location_Id WHERE (des.DesignationName LIKE ? OR cd.DEPT_NAME LIKE ? OR ct.TeacherID LIKE ? OR ct.FirstName LIKE ? OR ct.mobileNo LIKE ? OR ct.presentAddress LIKE ? OR ct.isActive LIKE ?) ");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearch : Ending");
...
//psmt2=conn.prepareStatement("SELECT DISTINCT ct.TeacherID,ct.FirstName,ct.department,cd.DEPT_NAME,ct.designation,des.designationName,cay.acadamic_id,cay.acadamic_year,cl.Location_Id,cl.Location_Name FROM campus_acadamicyear cay,campus_teachers ct LEFT JOIN campus_department cd ON cd.DEPT_ID=ct.department LEFT JOIN campus_designation des ON des.DesignationCode=ct.designation LEFT JOIN campus_location cl ON ct.Loc_ID=cl.Location_Id WHERE cay.acadamic_id LIKE ? AND (des.DesignationName LIKE ? OR cd.DEPT_NAME LIKE ? OR ct.TeacherID LIKE ? OR ct.FirstName LIKE ? OR ct.mobileNo LIKE ? OR ct.presentAddress LIKE ? OR ct.isActive LIKE ?) ORDER BY CAST(SUBSTR(cay.acadamic_id,4)AS UNSIGNED),CAST(SUBSTR(ct.Loc_ID,4)AS UNSIGNED),CAST(SUBSTR(cd.DEPT_ID,4)AS UNSIGNED),CAST(SUBSTR(des.DesignationCode,4)AS UNSIGNED)");
	...
	public List<PageFilterVo> getstaffSearchByLocation(String searchTerm,String locationId, String academicYear) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByLocation : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByLocation : Ending");
	...
	public List<PageFilterVo> getstaffSearchByDepartment(String searchTerm,String locationId, String academicYear, String department) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByDepartment : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByDepartment : Ending");
	...
	public List<PageFilterVo> getstaffSearchByAll(String searchTerm,String locationId, String academicYear, String department,String designation, UserLoggingsPojo userLoggingsVo) {
	
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByAll : Starting");
		...
		try{
			...
			psmt2=conn.prepareStatement("SELECT DISTINCT ct.registerId,ct.TeacherID,CONCAT(ct.FirstName,' ',ct.LastName)AS FirstName,ct.department,cd.DEPT_NAME,ct.presentAddress,ct.designation,des.designationName,cay.acadamic_id,cay.acadamic_year,cl.Location_Id,cl.Location_Name,CASE WHEN ct.isActive='Y' THEN 'ACTIVE' WHEN ct.isActive='N' THEN 'INACTIVE' ELSE ct.isActive END isActive,ct.mobileNo "+ 
										"FROM campus_acadamicyear cay,campus_teachers ct "+ 
										"LEFT JOIN campus_department cd ON cd.DEPT_ID=ct.department AND cd.`isActive` = 'Y' "+
										"LEFT JOIN campus_designation des ON des.DesignationCode=ct.designation AND des.`isActive` = 'Y' "+
										"LEFT JOIN campus_location cl ON ct.Loc_ID=cl.Location_Id AND cl.`isActive` = 'Y' "+
										"WHERE (des.DesignationName LIKE ? OR cd.DEPT_NAME LIKE ? OR ct.TeacherID LIKE ? OR ct.FirstName OR ct.LastName LIKE ? OR LIKE ? OR ct.mobileNo "+ 
										"LIKE ? OR ct.presentAddress LIKE ? OR ct.isActive LIKE ?) AND ct.Loc_ID = ? AND ct.department "+ 
										"LIKE ? AND des.DesignationCode LIKE ? AND cay.acadamic_id = ? AND  `dateofJoining` <=`endDate` ORDER BY CAST(registerId AS SIGNED),FirstName,`DEPT_NAME`,`designationName`");	
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByAll : Ending");
	...
	public List<PageFilterVo> getstaffSearchByDesig(String searchTerm,String locationId, String academicYear, String designation) {
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByDesig : Starting");
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in IDGenerator: getstaffSearchByDesig : Ending");
