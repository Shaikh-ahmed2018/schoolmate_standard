...
public class TeacherLeaveRequestServiceImpl implements TeacherLeaveRequestService {
	...
	public ArrayList<TimeTableVo> getTeacherTimetableListService(String userid,UserLoggingsPojo userLoggingsVo) {
	
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TeacherLeaveRequestServiceImpl : getTeacherTimetableListService Starting"); 
		

		ArrayList<TimeTableVo> uservo = null;
		try {
			
			uservo = dao. getTeacherTimetableListDao(userid,userLoggingsVo);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in TeacherLeaveRequestServiceImpl : getTeacherTimetableListService  Ending");
	...
	public String leaveRequestEntryService(LeaveRequestVo leavevo,UserLoggingsPojo custdetails) {
		...
		if(leavevo.getSno()==0){
			...
			try {
				leavevo1 = dao.leaveRequestEntryDao(leavevo,custdetails);
			...
			try {
				...
				leavevo1 = dao.updateleaveRequestEntryDao(leavevo,custdetails);
