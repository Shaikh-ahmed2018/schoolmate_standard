...
public class CareersViewServiceImpl implements CareersViewService {
	...
	public List<CareersViewVo> getAllCareersView(UserLoggingsPojo userLoggingsVo) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
		...
		getCareers = careersview.getAllCareersView(userLoggingsVo);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
	...
	public List<CareersViewVo> searchDetails(String name,UserLoggingsPojo userLoggingsVo) {
		...
		try {
			careersview = new CareersViewDaoImpl();
			list = careersview.searchDetails(name,userLoggingsVo);
		} catch (Exception e) {
			logger.error(e);
			e.printStackTrace();
		}
