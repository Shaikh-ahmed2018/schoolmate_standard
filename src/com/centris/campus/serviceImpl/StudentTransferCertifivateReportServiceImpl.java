...
public class StudentTransferCertifivateReportServiceImpl implements StudentTransferCertifivateReportService{
	...
	public List<ClassPojo> getClassDetailsService(UserLoggingsPojo userLoggingsVo) {
		...
		try {
			pojo = dao.getClassDetailsDao(userLoggingsVo);
	...
	public List<SectionPojo> getSectionListService(String classname,String location,UserLoggingsPojo userLoggingsVo) {
		
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentTransferCertifivateReportServiceImpl : getSectionListService Starting");
		
		List<SectionPojo> seclist = null;
		
		try {
			
			seclist = dao.getSectionListDao(classname,location,userLoggingsVo);
		...
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.END_POINT);
		logger.info(JDate.getTimeString(new Date())
				+ " Control in StudentTransferCertifivateReportServiceImpl : getSectionListService Ending");
