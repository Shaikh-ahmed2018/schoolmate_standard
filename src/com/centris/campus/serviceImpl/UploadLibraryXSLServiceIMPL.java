...
public class UploadLibraryXSLServiceIMPL implements UploadLibraryXSLservice {
	...
	public Set<UploadLibraryXlsVO> insertLibXSL(List<UploadLibraryXlsPOJO> list, String username, String duplicate,String currentLoc, String log_audit_session)
	{
		...
		try {
			...
			if(successlist.size()>0){
				failureListFromDiompl=daoImpl.insertLibXSL(successlist,connection,username,log_audit_session);
			}
	...
	public Set<UploadLibraryXlsVO> insertsubscriberDetailXSL(List<UploadLibraryXlsPOJO> list,String username,String demo,String schoolLocation, String log_audit_session, UserLoggingsPojo custdetails) {
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		logger.info(JDate.getTimeString(new Date())
		...
		try {
			...
			String liblocid="notfound",stuId=null,academicYearId="empty";
			String int_regex="^[0-9a-zA-Z]*$";
			String status="^[a-zA-Z]*$";
			...
			*/
			...
			for (Iterator<UploadLibraryXlsPOJO> iterator = list.iterator(); iterator.hasNext();) {
				...
				if(UploadLibraryXlsPOJO.getAcademicYearId()!=null){
					if(UploadLibraryXlsPOJO.getAcademicYearId().trim()!=""){
						academicYearId=daoImpl.getAcademicYearId(UploadLibraryXlsPOJO.getAcademicYearId(),connection);
					}
				}
				...
				if(!liblocid.equalsIgnoreCase("notfound") && liblocid!=null){
					...
					if(UploadLibraryXlsPOJO.getSubscriberType().equalsIgnoreCase("student") && 
							!UploadLibraryXlsPOJO.getAdmissionNumber().equalsIgnoreCase(""))
					{
						pstmt =connection.prepareStatement("SELECT student_id_int FROM campus_student WHERE student_admissionno_var=? AND student_status_var='active'"); /*AND fms_acadamicyear_id_int=?*/
						pstmt.setString(1, UploadLibraryXlsPOJO.getAdmissionNumber()); 
						/*pstmt.setString(2, academicYearId);*/
					...
					{
						pstmt =connection.prepareStatement("SELECT TeacherID FROM campus_teachers WHERE registerId=? AND isActive='Y'");
						pstmt.setString(1, UploadLibraryXlsPOJO.getAdmissionNumber());
					...
					else if(academicYearId.equalsIgnoreCase("empty")){
						uploadLibraryXSLVo.setReasone("Academic Year Should Not Empty");
						failurelist.add(uploadLibraryXSLVo);
					}
					 
					else if(academicYearId.equalsIgnoreCase("notfound")){
						uploadLibraryXSLVo.setReasone("Academic Year doesn't Exists");
						failurelist.add(uploadLibraryXSLVo);
					} 
					...
					{
						uploadLibraryXSLVo.setReasone("Duplicate Subscriber Number");
						failurelist.add(uploadLibraryXSLVo);
					}
					...
					else if(UploadLibraryXlsPOJO.getDepositType().equalsIgnoreCase("cash") && UploadLibraryXlsPOJO.getAmount().equalsIgnoreCase(" "))
					{
						...
						uploadLibraryXSLVo.setReasone("Please Enter the valid Status(PAID / UNPAID)");
						failurelist.add(uploadLibraryXSLVo);
					}
					else if(UploadLibraryXlsPOJO.getDate()=="" || UploadLibraryXlsPOJO.getDate().equals("-")){
						uploadLibraryXSLVo.setReasone("Subscription Date Should Not Empty ");
						failurelist.add(uploadLibraryXSLVo);
					}
					else if(UploadLibraryXlsPOJO.getDate()=="" || !UploadLibraryXlsPOJO.getDate().matches(datePattern)){
						 
						uploadLibraryXSLVo.setReasone("Invalid Subscription Date Entry ");
						failurelist.add(uploadLibraryXSLVo);
	              	}
						...
						UploadLibraryXlsPOJO.setAcademicYearId(academicYearId);
						successlist.add(UploadLibraryXlsPOJO);
					}
			...
			Set<UploadLibraryXlsVO> failureListFromDiompl=new LinkedHashSet<UploadLibraryXlsVO>();
			if(successlist.size()>0){
				failureListFromDiompl=daoImpl.insertsubscriberDetailXSL(successlist,connection,log_audit_session,custdetails);
			}
			for (Iterator<UploadLibraryXlsVO> it = failureListFromDiompl.iterator(); it.hasNext(); ) {
				UploadLibraryXlsVO failureDiomplVo = (UploadLibraryXlsVO) it.next();
				UploadLibraryXlsVO uploadLibraryXSLVo = new UploadLibraryXlsVO();
				...
				uploadLibraryXSLVo.setAcademicYearId(failureDiomplVo.getAcademicYearId());
	...
	public Set<StockEntryVo> insertStockEntryXSL(List<StockEntryPOJO> list, String username, 
			String duplicate,String currentLoc, String log_audit_session, UserLoggingsPojo custdetails, String academic_year)
	{
		logger.setLevel(Level.DEBUG);
		JLogger.log(0, JDate.getTimeString(new Date())
				+ MessageConstants.START_POINT);
		...
		PreparedStatement pstmt=null,pstmt1=null,pstmt2=null,pstmt3=null,pstmt4=null,
				pstmt5=null,pstmt6=null,pstmt7=null,pstmt8=null,pstmt9=null,pstmt10=null;
		ResultSet rs=null,rs1=null,rs4=null,rs2=null,rs3=null,rs7=null,rs8=null,rs6=null,rs9=null,rs10=null;
		...
		boolean pancardflag=false;
		boolean personalemailflag=false;
		boolean officialemailflag=false,regDate=false,billDate=false,currentStautsCheck=true;
		int checkCategoryTypeName=0;
		...
		try {
			...
			for (Iterator<StockEntryPOJO> iterator = list.iterator(); iterator.hasNext();) {
				...
				if(StockEntryxLSPOJO.getRegisterdDate()!=null){
					if(!StockEntryxLSPOJO.getRegisterdDate().trim().equals(""))
					{
						regDate = HelperClass.DateValidator(StockEntryxLSPOJO.getRegisterdDate().replace("-","/"));
						if(regDate == true)
							StockEntryxLSPOJO.setRegisterdDate(StockEntryxLSPOJO.getRegisterdDate().replace("/","-"));
					}
				} 
				
				if(StockEntryxLSPOJO.getBilldate()!=null){
					if(!StockEntryxLSPOJO.getBilldate().trim().equals(""))
					{
						billDate = HelperClass.DateValidator(StockEntryxLSPOJO.getBilldate().replace("-","/"));
						if(billDate == true)
							StockEntryxLSPOJO.setBilldate(StockEntryxLSPOJO.getBilldate().replace("/","-"));
					}
				}
				
				if(StockEntryxLSPOJO.getCurrentstatus()!=null){
					if(!StockEntryxLSPOJO.getCurrentstatus().trim().equals(""))
					{
						if(StockEntryxLSPOJO.getCurrentstatus().equalsIgnoreCase("Available") ||
								StockEntryxLSPOJO.getCurrentstatus().equalsIgnoreCase("Unavailable")){
							currentStautsCheck=false;
						}else{
			     ...
			     pstmt9.setString(1,StockEntryxLSPOJO.getIsbnno());
			     pstmt9.setString(2,LibraryLocation);
			     ...
			     pstmt10.setString(1,StockEntryxLSPOJO.getBillno());
			     pstmt10.setString(2,LibraryLocation);

			     rs10=pstmt10.executeQuery();
			     while(rs10.next()){
			    	 count1=rs10.getInt(1);
			     }
				...
				/*else if(Category.equalsIgnoreCase("no")){
					...
					UploadStockEntryXLSVo.setReason("Reg Date should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getBooktitle()==""){
					UploadStockEntryXLSVo.setReason("Book Title should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(regDate==false){
					UploadStockEntryXLSVo.setReason("Invalid Reg Date");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(StockEntryPOJO.getAuthor()==""){
					UploadStockEntryXLSVo.setReason("Author should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
					...
					UploadStockEntryXLSVo.setReason("Language should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getPublisher()==""){
					UploadStockEntryXLSVo.setReason("Publisher should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getNoofcopies()==""){
					UploadStockEntryXLSVo.setReason("No of copies should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getCostpercopy()==""){
					UploadStockEntryXLSVo.setReason("Cost should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getEdition()==""){
					UploadStockEntryXLSVo.setReason("Edition should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getIsbnno()=="" && UploadStockEntryXLSVo.getItemtype().equalsIgnoreCase("book")){
					UploadStockEntryXLSVo.setReason("ISBN No should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}
				else if(count1>0 && UploadStockEntryXLSVo.getItemtype().equalsIgnoreCase("book")){
					UploadStockEntryXLSVo.setReason("ISBN No already exist");
					failurelist.add(UploadStockEntryXLSVo);
				}
				else if(UploadStockEntryXLSVo.getSize()==""){
					UploadStockEntryXLSVo.setReason("Size should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getSuppliedby()==""){
					UploadStockEntryXLSVo.setReason("Supplier should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getGeneralinfo()==""){
					UploadStockEntryXLSVo.setReason("General Info should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getPublicationyear()==""){
					UploadStockEntryXLSVo.setReason("Publication Year should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}
				else if(!UploadStockEntryXLSVo.getPublicationyear().matches(validyear)){
					UploadStockEntryXLSVo.setReason("Enter valid Publication Year");
					failurelist.add(UploadStockEntryXLSVo);
				}
				else if(UploadStockEntryXLSVo.getTotalcost()==""){
					UploadStockEntryXLSVo.setReason("Total Cost should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				} 
				else if(count2>0){
					UploadStockEntryXLSVo.setReason("Bill No already exist");
					failurelist.add(UploadStockEntryXLSVo);
				}
				else if(UploadStockEntryXLSVo.getEditor()==""){
					UploadStockEntryXLSVo.setReason("Editor should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getBilldate()==""){
					UploadStockEntryXLSVo.setReason("Bill Date should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(billDate==false){
					UploadStockEntryXLSVo.setReason("Invalid Bill Date");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getContentsearch()==""){
					UploadStockEntryXLSVo.setReason("content should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getShelfno()==""){
					UploadStockEntryXLSVo.setReason("Shelf No should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}else if(UploadStockEntryXLSVo.getLibrarylocation()==""){
					UploadStockEntryXLSVo.setReason("Library Location should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				}
				else if(UploadStockEntryXLSVo.getCurrentstatus()==""){
					UploadStockEntryXLSVo.setReason("Current Status should not be empty");
					failurelist.add(UploadStockEntryXLSVo);
				} 
				else if(currentStautsCheck){
					UploadStockEntryXLSVo.setReason("Enter valid Current Status (Available / Unavailable)");
					failurelist.add(UploadStockEntryXLSVo);
				}
			...
			if(successlist.size()>0){
				failureListFromDiompl=daoImpl.insertStockEntryXSL(successlist,conn,username,log_audit_session,
						custdetails,academic_year,LibraryLocation);
			}
