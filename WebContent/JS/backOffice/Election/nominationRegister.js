			...
			$("#admissionNo").autocomplete({
				source : function(request, response) {
					...
					$.ajax({
						...
						url : "ElectionMenu.html?method=studentSearchbyadmissionNo",
						data : {
...
/*	 	var admissionNo = $("#admissionNo").val().trim();
		...
		var groupId=$("#groupName").val().trim();
		var titleId=$("#electionTitle").val();
		var accyearId =$("#academicYear").val().trim();
		...
		success : function(data){
			...
			if(result.status=="true"){
				...
				setTimeout(function() {
					...
					window.location.href="menuslist.html?method=nominationRegister";
			}, 2000);
		 }	
