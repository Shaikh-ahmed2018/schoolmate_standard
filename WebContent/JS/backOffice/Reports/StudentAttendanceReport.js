$(document).ready(function() {
	...
	$('#section').change(function() {
		var childnameid = $("#studentname").val();
		getStudentName(childnameid);
	});
...
function getStudentName(childnameid) {
	...
	$.ajax({
		type : 'POST',
		url : "studentattendanceReport.html?method=getAllStudentName",
		data : datalist,
		async : false,
		success : function(response) {
...
/*	$(childnameid).html("");*/
	...
	for ( var j = 0; j < result.parentVOList.length; j++) {
		$('#studentname')
				.append(
						'<option value="'
								+ result.parentVOList[j].studentid
								+ '">'
								+ result.parentVOList[j].studentFnameVar
								+ '</option>');
	}
