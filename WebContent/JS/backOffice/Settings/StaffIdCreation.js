...
$(document).ready(function () {
	...
	$("#back1").click(function(){
		window.location.href = "menuslist.html?method=DesignStaffIDCard&historyaccyear="+$("#historyaccyear").val()+"&historyschoolId="+$("#historyschoolId").val()+"&historydepartmentId="+$("#historydepartmentId").val();
	});
	
	
	getStaffIdDesignList();
	...
	$("#schoolName").dblclick(function(){
		...
		$("#saveChanges").click(function(){
			...
			var mainCss=cssgroup.toString().replace(/,/g,"");
			 $.ajax({
				 type: 'POST', 
				  url: "menuslist.html?method=StaffIdDesign",
		...
		$("#main-div span").bind("contextmenu",function(e){
			...
			$("#main-div div[id]").bind("contextmenu",function(e){
				...
				$("#idCardDesignListDropDown").change(function(){
					if(this.value !=""){
					$('body').append('<link href="CSS/IdCard/StaffId/'+$(this).val()+'" rel="stylesheet" type="text/css" />');
					
					$("#layoutImage").attr("src","./images/IdCard/"+$(this).val().split(".")[0]+".jpg");
					
					if(tempTemplateName!=null){
						$('body').find("link[href='CSS/IdCard/StaffId/"+tempTemplateName+"']").remove();
					}
...
function getStaffIdDesignList(){
	$.ajax({
		type:"POST",
		url:"menuslist.html?method=getStaffIdDesignList",
		async:false,
