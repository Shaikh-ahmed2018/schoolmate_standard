...
$(document).ready(function(){
	...
	$("#saveid").click(function(){
		...
		if(year == "" || year == null){
			...
			$('table#addholiday tr[id]').each(function() {
		 		 ...
		 		 type: "GET",
		           ...
		           async:false,
					...
					$(".validateTips").text("Adding record progressing...");		
		 				...
		 				$(".errormessagediv").show();
						$(".validateTips").text("Failed to add holiday. Trt again");
		 			}
					setTimeout(function() {
		 			window.location.href="menuslist.html?method=holidaymaster";
					},2000);		 
		 		  }
		 	 });
 	     }
...
function addMoreHoliday(frm) {
	...
	$('#addholiday tr')
			...
			.after(
					'<tr id="holidayId'
							+ rowHoliday
						    + '"><td><input type="text" name="date" readonly="readonly" onclick="HideError(this)" class="form-control date"  placeholder="Holiday Date" type="text"/></td><td><input type="text" name="weekday" onclick="HideError(this)" readonly="readonly" class="form-control"  /></td><td><input type="text" onclick="HideError(this)" name="holiday" id="holiday'+rowHoliday+'" class="form-control holiday" /></td><td ><select name="holidaytype" onkeypress="HideError(this)" class="form-control holidaytype" style="min-width:150px;"><option value="">----Select----</option><option value="FirstHalf">First Half</option><option value="Second Half">Second Half</option><option value="Full Day">Full Day</option></select></td>        <td align="center"><a href="javascript:void(0);" class="glyphicon glyphicon-trash" onclick="removeHoliday('
						    + rowHoliday +');"></a></td></tr>');
	...
	weekday[5]="Saturday";
	weekday[6]="Sunday";
	fSDate = new Date($("#accstdate").val());
	fEDate = new Date($("#accenddate").val());
	   ...
	   onClose:function(){
		  var thispointer=$(this);
			...
			$("#accyYear").change(function(){
				   ...
				   $("#addholiday tbody tr").not(".firstrow").empty();
				   addMoreHoliday();
				$("#addholiday tbody tr[id]").each(function(){
					holidayValidate($(this).find(".holiday"), $("#locationname").val(),$("#accyYear").val());
					DateValidate($(this).find("input[name='date']"),$("#locationname").val(),$("#accyYear").val());
			...
			success: function(data) {
			 var result=$.parseJSON(data);	
				if(result.dateVal=='exit')
				{
					 ...
					 $(".errormessagediv").show();
					 errorMsg="Date Already Exists, Select Another Date";
					$(".validateTips").text("Date "+$(dateVal).val()+" Already Exists, Select Another Holiday");
...
function getAcademicYearStartDate(){
	...
	$.ajax({
		...
		var  result=$.parseJSON(response);
			...
			$("#accstdate").val(result.startdate);
			$("#accenddate").val(result.enddate);
