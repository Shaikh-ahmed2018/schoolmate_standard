...
$(document).ready(function(){
	...
	$("#back1").click(function(){
		//window.location.href="menuslist.html?method=gradeList&historyacademicId="+$("#historyacademicId").val()+"&historylocId="+$("#historylocId").val();
		
		$('#myValue').val("historyacademicId="+$("#historyacademicId").val()+"&historylocId="+$("#historylocId").val());
		$('#myform').submit();
	});
	...
	$("#dialog").dialog({
		    ...
		    modal     : true,
		    width	:400,
		    title     : "New Grade Details",
		    buttons   : {
		    	"Save": {
		    		text: 'Save',
		    		 click: function() {
		    			 var grade = $("#gradeName").val().trim();
		    			 var comments = $("#comments").val().trim();
		    			 var minmarks = $("#minmarks").val().trim();
		    			 var maxmarks = $("#maxmarks").val().trim();
		    			 var accyear = $("#hiddenaccyid").val().trim();
		...
		success : function(data){
			...
			for(var i=0;i<result.gradeSettingsList.length;i++){
				...
				$("#markstable #allstudent").append(
						...
						+"<td style='padding-bottom: 9px;padding-top: 9px;'><span class='buttons editing'>Modify</span><span class='buttons aligning' style='margin-left:10px;'>Remove</span></td>"
						+"</tr>"
				);
...
$(".aligning").click(function(){
		...
		id=$(this).closest("tr").find(".gradeid").attr("id");
