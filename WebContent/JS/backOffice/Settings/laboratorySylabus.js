			...
			success : function(data) {
				...
				var result = $.parseJSON(data);
					...
					$("#allstudent tbody").append("<tr>"
							...
							+"<td><div><input type='file' name='uploadLabSylabus' class ='uploadLabSylabus' /></div></td>"
...
$(document).on('change', '.uploadLabSylabus', function(e){
	        	...
	        	$.ajax({
	  		      ...
	  		      success : function(data) {
	  					...
	  					if(result.status == "fail"){
	  						...
	  						$(".validateTips").text("Failed to Upload Syllabus...");
	  					}else{
...
$(document).on('click', '.downloadSyllabus', function(e){
  ...
  file1=$(this)[row].files[0];
	  ...
	  {
	  formdata.append("addSyllabus",file1);
	  formdata.append("subId", tr.find('.subId').text());
	  formdata.append("locationId", $("#locationname").val());
	  formdata.append("academicYear", $("#academicYear").val());
	  formdata.append("classname", $("#classname").val());
	  formdata.append("clas", $("#classname").text());
	  formdata.append("sub", tr.find('.subName').text());
	  formdata.append("type","S");
		 ...
		 $.ajax({
		      type: "POST",
		      url: "subject.html?method=syllabusDownload",
		      data: formdata,
              enctype: 'multipart/form-data',
              processData: false,
              contentType: false,
		      success : function(data) {
					...
					if(result.status == "fail"){
						$('.errormessagediv').show();
						$(".successmessagediv").hide();
						$(".validateTips").text("Syllabus  Download Failed...");
					}else{
						$('.errormessagediv').hide();
						$(".successmessagediv").show();
						$(".validateTips").text("Download Successfull..");
						setTimeout(function(){
							getSyllabusforPrint($("#locationname").val(),$("#classname").val(),$("#specialization").val(),$("#academicYear").val());
						},2000);
					}
  ...
  $.ajax({
    ...
    success:function(response) {
    	...
    	if(result.status==true){
			...
			$('.errormessagediv').show();
			$('.validateTips').text("Selected subject cannot be"+" "+$("#delete").text());
			$(".errormessagediv").css({
				'z-index':'9999999'
			});
		 }
