...
$(document).ready(function(){
	...
	if(pageUrl=="settingXLUpload.html?method=ReligionExcelInsert"){
	...
	$("#Religionfile").change(function(){
		$("#saveid").show();
	});
	
	$("#saveid").click(function(){
	var filename = $("#Religionfile").val().split(".").pop().toLowerCase();
	var fileNameCheck=$("#Religionfile").val();
	
	if(filename=="xlsx" || filename=="xls"){
		$("#Add").val("xls");
	 		$(".errormessagediv").hide();
	  		$("#Religionfile").css({'backgroundColor' : 'transparent','border':'1px solid #ccc'});
			return true;
		}else{
			 $(".errormessagediv").show();
				$(".validateTips").text("Select Excel File Only");
				 $("#Religionfile").css({'backgroundColor' : '#FFF7F7','border-color':'#B70606'});
...
$("#Religionfile").change(function(){
	var filename = $("#Religionfile").val().split(".").pop().toLowerCase();
	if(filename=="xlsx" || filename=="xls"){
	$("#Religionfile").css({'backgroundColor' : 'transparent','border-color':'#ccc'});
	}
	else{
	$("#Religionfile").css({'backgroundColor' : '#FFF7F7','border-color':'#B70606'});
	}
