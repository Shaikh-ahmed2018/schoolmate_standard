				...
				function() {
					...
					$("#save").click(function() {
										...
										$('table#allstudent tr:gt(0)').each(function() {
											...
											var examName = $(this).find('[id=subName]').val();
											var examDate = $(this).find('[name=startDate]').val();
											var startTime = $(this).find('[id=startTime]').val();
											var endTime = $(this).find('[id=endTime]').val();

											var examdate = Date.parse(dateConverter(examDate));
											var startdate = Date.parse(dateConverter(stDate));
											var enddate = Date.parse(dateConverter(endDate));
                                             
											if (examDate.trim() == "") {
												$(".errormessagediv1").show();
												$(".validateTips").text(" Exam Date should not be empty for "+examName+" subject");

												status = false;
												return false;
											} else if (startdate > examdate || examdate > enddate) {
												$(".errormessagediv1").show();
												$(".validateTips").text("Exam Date should be lie between startdate("+$("#startDate").val()+") and enddate("+$("#endDate").val()+") for "+ examName+" subject");
												...
												status = false;
												return false;
											} else if (examDate.trim() != '') {
												status = true;
												$(".errormessagediv1").hide();
											}
											if (startTime.trim() == "") {
												$(".errormessagediv1").show();
												$(".validateTips").text("Select Start time for "+ examName+ " subject");
												status = false;
												return false;
											...
											if (endTime.trim() == "") {
												$(".errormessagediv1").show();
												$(".validateTips").text("Select End time for "+ examName+ " subject");
												status = false;
												return false;
											} else if (checkTime(startTime.trim(),endTime.trim())) {
												$(".errormessagediv1").show();
												$(".validateTips").text("The End Time Should Not less than Start Time for "+ examName+ " subject");
												status = false;
												return false;

											} else if (endTime.trim() != undefined && endTime.trim() != '') {
												status = true;
												$(".errormessagediv1").hide();
											}
											rc++;
										...
										if (status == true) {
											...
											if (answer) {
												...
												var subjectId = [];
												$('table#allstudent  td.subjectid').map(function() {
															subjectId.push($(this).text().trim());
											    }).get();

												var date = [];
												$('table#allstudent  input.datepicker').map(function() {

																	date.push($(this).val().trim());
												}).get();

												var time = [];
												$('table#allstudent  input.timepicker').map(function() {

																	time.push($(this).val().trim());
												}).get();

												var endtime = [];
												$('table#allstudent  input.endtimepicker').map(function() {

																	endtime.push($(this).val().trim());
												}).get();
												...
												$.ajax({
															...
															data : {
																"classId" : classId,
																"examId" : examId,
																"subjectid_str" : subjectId.join(","),
																"date_str" : date.join(","),
																"time_str" : time.join(","),
																"end_time_str" : endtime.join(",")
															},
															async : false,
