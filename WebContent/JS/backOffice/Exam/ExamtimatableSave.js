...
$(document).ready(function(){
	...
	$("#back1").click(function(){
		...
		"&classname="+$("#class").val()+"&examid="+$("#hiddenexamid").val()+
		"&historyaccyear="+$("#historyaccyear").val()+"&historylocation="+$("#historylocation").val()
		+"&historysection="+$("#historysection").val()+"&historyexam="+$("#historyexam").val();*/
	...
	$('.endtime').change(function(){
		...
		var currenth=$(this);
		var date=$(this).closest('tr').find(".startdate").val();
...
$("#allstudent tbody tr[id]").find(".endtime").not(currenth).each(function(){
	...
	if(($(this).closest('tr').find(".startdate").not(currenth.closest('tr').find('.startdate')).val() == currenth.closest('tr').find(".startdate").val())){
		if( endtime <= $(this).closest('tr').find(".endtime").val() && endtime >=$(this).closest('tr').find(".starttime").val()){
			timeFlag=true;
			return false;
		}
		/*else if(currenth.closest('tr').find(".starttime").val() >= starttime && currenth.closest('tr').find(".starttime").val() < endtime){
...
$("#allstudent tbody tr[id]").find(".starttime").not(currenth).each(function(){
	...
	/*
	alert(starttime+" asfa "+ $(this).closest('tr').find(".endtime").not(currenth.closest('tr').find('.endtime')).val());
	...
	$("#save").click(function(){
		...
		$('table#allstudent tr[id]').each(function(){
			...
			if(endtime <= starttime){
				$(".errormessagediv").show();
				$(".validateTips").text("End Time must be greater than Start Time.");
				$(this).find('[name=endtime]').css({'border':'1px solid #AF2C2C'});
				setTimeout(function() {
					$('.errormessagediv').fadeOut();
				}, 3000);
