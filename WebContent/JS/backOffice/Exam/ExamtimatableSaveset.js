...
$(document).ready(function(){
	...
	$("#back1").click(function(){
		/*window.location.href ="examTimetablePath.html?method=getexamtimetableclasssection&accyear="+$("#hiddenaccyear").val()
		+"&locid="+$("#hiddenloc").val()+"&classid="+$("#hiddenclass").val()+"&classname="+$("#class").val()
		+"&examid="+$("#hiddenexamid").val()+"&historyaccyear="+$("#historyaccyear").val()
		+"&historylocation="+$("#historylocation").val()+"&historysection="+$("#historysection").val()
		+"&historyexam="+$("#historyexam").val();*/
		
		$('#myValue1').val($("#hiddenaccyear").val()+","+$("#hiddenloc").val()+","+$("#hiddenclass").val()+","+$("#hiddenexamid").val()+","+$("#historyaccyear").val()+","+$("#historylocation").val()+","+$("#historysection").val()+","+$("#historyexam").val());
		$('#myform1').submit();
	...
	$(".startdate").change(function(){
		...
		if(timeFlag){
			...
			$(".errormessagediv .validateTips").text("Start Time Should be Different");
	...
	$('.endtime').change(function(){
		...
		$("#allstudent tbody tr[id]").find(".endtime").not(currenth).each(function(){
			if(currenth.closest('tr').attr('class')=="Y"){
				if((currenth.closest('tr').attr('class')==$(this).closest('tr').attr('class')) && !language){
					language=true;
	...
	$("#allstudent tbody tr[id]").find(".starttime").not(currenth).each(function(){
		...
		if(starttime >= $(this).val() && $(this).val()<$(this).closest('tr').find(".endtime").not(currenth.closest('tr').find('.endtime')).val()){
			timeFlag=true;
			return false;
		}
		if(starttime <=$(this).val() && $(this).val()<$(this).closest('tr').find(".endtime").not(currenth.closest('tr').find('.endtime')).val()){
	...
	$("#save").click(function(){
		...
		datalist = {
				...
				"sectionid" :sectionid,
				 ...
				 "endtime1" :endtime1,
				 "timetable" : $("#hiddentimeid").val()
		};
		if(saveflag){
				$.ajax({
			 		 type: "POST",
			 		  url: "examTimetablePath.html?method=savetimetabledetailsset",
			           data:datalist,
			           async:false,
						...
						$(".validateTips").text("Updating Record progressing...");		
			 				...
			 				$(".errormessagediv").show();
							$(".validateTips").text("Record Not Updated.");
			 			}
						setTimeout(function() {
							/*window.location.href ="examTimetablePath.html?method=getexamtimetableclasssection&accyear="+$("#hiddenaccyear").val()
							+"&locid="+$("#hiddenloc").val()+"&classid="+$("#hiddenclass").val()+"&classname="+$("#class").val()
...
function getsubdetails(locid,classid){
		...
		$.ajax({
			type : 'POST',
			url : "examTimetablePath.html?method=getsubdetailsset",
			data : {"location":$("#hiddenloc").val(),"classid":$("#hiddenclass").val(),"examid":$("#hiddenexamid").val(),"accyear":$("#hiddenaccyear").val(),"timetableid":$("#hiddentimeid").val()},
			async : false,
				...
				var result = $.parseJSON(response);
					...
					for(var i=0;i<result.sublist.length;i++){
						...
						$("#allstudent tbody").append("<tr id='"+result.sublist[i].sno1+"' class='"+result.sublist[i].islanguage+"' >"
							...
							+"<td><input type='text' name='startdate' readonly='readonly' id='startdate"+rowdate+"' class='form-control date startdate'  placeholder='Select Date' value='"+result.sublist[i].startDate+"'/></td></td>"
							...
							+"<input id='starttime"+result.sublist[i].sno1+"' class='form-control inputcolor starttime'  type='text' name='starttime' value='' readonly>"
							...
							+"<input id='endtime"+result.sublist[i].sno1+"' class='form-control endtime'  type='text' name='endtime' value='' readonly>"
					...
					for(var i=0;i<result.sublist.length;i++){
						$("#starttime"+(i+1)).val(result.sublist[i].examstarttime);
						$("#endtime"+(i+1)).val(result.sublist[i].examendtime);
					}
					...
					pagination(50);
					$(".numberOfItem").empty();
					$(".numberOfItem").append("  No. Of Records "+result.sublist.length);
