				...
				$("#locationName").val()==undefined)
 	   {
			$(".errormessagediv").show();
   			...
   			return false;
	   			...
	   			"&itemType1="+$("#itemType option:selected").text()+
	   			"&bookTitle="+$("#bookTitle option:selected").text()+
	   			"&author="+$("#author option:selected").text()+"&publishernm="+$("#publisher option:selected").text()+'&schId='+$("#schoolName").val()+'&schname='+$("#schoolName option:selected").text();
		}
	});
	...
	$.ajax({
		...
		success : function(response) {
			...
			for(var i=0;i<result.itemlist.length;i++){
				 $("#itemType").append("<option value='"+result.itemlist[i].itemType+"'>"+result.itemlist[i].item+"</option>");
			}
		}
	});
