		...
		.ready(
				function() {
					...
					$("#rst")
							.click(
									function() {
										...
										window.location.href = "studentcertificate.html?method=ageCertificate&locationid="
					...
					$("#back1")
							.click(
									function() {
										window.location.href = "studentcertificate.html?method=studentCertificates&historyaccyear="
												+ $("#historyaccyear").val()
												+ "&historylocation="
												+ $("#historylocation").val()
												+ "&historyclassid="
					...
					$("#save")
							.click(
									function() {
										...
										datalist = {
											"accyear" : $("#hiddenaccyearid")
													.val(),
											"locid" : $("#hiddenlocationid")
													.val(),
											"classid" : $("#hiddenclassid")
													.val(),
											...
											"Save" : {
												...
												click : function() {
													...
													$('input[type=text]').attr(
															'readonly', 'true');
													document
															.getElementById("purpose").style.backgroundColor = "#FFF";
													document
															.getElementById("otherinfo").style.backgroundColor = "#FFF";
													datalist = {
																					...
																					$
																							.ajax({
																								...
																								success : function(
																									...
																									for (var i = 0; i < result.stuList.length; i++) {
																										$(
																												...
																												.append(
																														"<tr>"
																																+ "<td class='ageid' id='"
																																+ result.stuList[i].agecertid
																																+ "'>"
																																+ result.stuList[i].slno
																																+ "</td>"
																																+ "<td>"
																																+ result.stuList[i].entryDate
																																+ "</td>"
																																+ "<td>"
																																+ result.stuList[i].purpose
																																+ "</td>"
																																+ "</tr>");
																									}
																									...
																									/*
																									 * pagination(50);
																									 * $("#certificatetablepage").append("<div
																									 * class='pagebanner'><select
																									 * id='show_per_page'><option
		...
		success : function(data) {
			...
			if (result.stuList.length == 0) {
				...
				$("#certificatetable").append(
						"<table class='table studenttable' id='allstudent' width='100%'"
								+ ">" + "<center><tr><th>SI No</th>"
								+ "<th>Entry Date</th>" + "<th>Purpose</th>"
								+ "</center></tr>" + "</table>");

				for (var i = 0; i < result.stuList.length; i++) {
					$("#allstudent tbody").append(
							"<tr>" + "<td class='ageid' id='"
									+ result.stuList[i].agecertid + "'>"
									+ result.stuList[i].slno + "</td>" + "<td>"
									+ result.stuList[i].entryDate + "</td>"
									+ "<td>" + result.stuList[i].purpose
									+ "</td>" + "</tr>");
				}
				$("#allstudent tbody tr").click(function(e) {
					e.preventDefault();
				...
				/*
				 * pagination(50); $("#allstudent").after("<div
				 * class='pagebanner'><select id='show_per_page'><option
				 * value='50'>50</option><option value='100'>100</option><option
				 * value='200'>200</option><option value='300'>300</option><option
				 * value='400'>400</option><option value='500'>500</option></select></div><div
				 * class='pagination pagelinks'></div>")
				 * $("#show_per_page").change(function(){
				 * pagination($(this).val()); });
				 */
			}
...
function pagination(list) {
	...
	while (number_of_pages > current_link) {
		navigation_html += '<a class="page" onclick="go_to_page('
	...
	$('#allstudent tbody tr,.allstudent tbody tr').slice(0, show_per_page).css(
			'display', 'table-row');
...
function go_to_page(page_num) {
	...
	$(".controls").find(".active").next().next().show();
	$('#allstudent tbody tr,.allstudent tbody tr').css('display', 'none')
			.slice(start_from, end_on).css('display', 'table-row');
...
function singleprint() {
	...
	var city = $("#hiddenAddres2").val().split(",")[1];
	var state = $("#hiddenAddres2").val().split(",")[0];
	...
	frameDoc.document
			.write('<link rel="stylesheet" href="JQUERY/development-bundle/themes/base/jquery.ui.all.css" />');
	frameDoc.document
			.write('<link href="CSS/newUI/bootstrap.min.css" rel="stylesheet">');
	frameDoc.document
			.write('<link href="CSS/newUI/modern-business.css" rel="stylesheet">');
	frameDoc.document
			.write('<link href="CSS/Certificates/ageCertificate.css" rel="stylesheet">');
	frameDoc.document.write('</head><body>');

	// Append the DIV contents.

	frameDoc.document
			.write('<div class="container"><div style="margin-left: 10px;margin-right: 10px;margin-top: 10px;margin-bottom: 10px;"><div class="row"><div col-md-12><div class="col-md-3"></div><div col-md-6><div style="margin:auto;">'
					+ '<table class="table"  id="bonafied"style="border:none;">'
					+ '<tr><td><img src="'
					+ schoolLogo
					+ '"  width="100" height="60"></img></td><td><p style="font-size: 29px;font-weight: bold;text-align:center;">'
					+ schooName
					+ '</p><p style="font-size: 20px;font-weight: bold;text-align:center;"><i>'
					+ adress
					+ '</i></p></td></tr></table><p style="font-size: 20px;font-weight: bold;text-align:center;margin-top: -5%;"></p>'
					+ '<hr style="margin-top: 0px; margin-bottom: 0px; border-color: black"><p style="text-align: center;font-size:25px;font-weight: bold; margin-bottom:-5px;"><i>AGE CERTIFICATE</i></p>'
					+ '<hr style="margin-top: 0px; margin-bottom: 0px; border-color: black;"><div style="margin-left: 11%; margin-top: 10px;">&nbsp;&nbsp;&nbsp;'
					+ '<div margin-left:-50px; margin-right:-50px;><table width="100%" id="Certificate"><tr><td colspan="2"> <p style="font-size:25px!important;">This is to certify that&nbsp<i>&nbsp;'
					+ stuname
					+ '</i>&nbsp&nbsp&nbsp<div style="width:65%;border-bottom: 1px dashed;display: block; margin-left:35%; margin-top: -10px;"></div></p></td></tr><tr><td colspan="2"><p style="margin-left:-50px;font-size:25px!important;">'
					+ rel
					+ ' of &nbsp<i>&nbsp;'
					+ fathername
					+ '</i><div style="width:96%;border-bottom: 1px dashed;display: block; margin-left:4%; margin-top: -10px;"></div></p></td></tr>'
					+ '<tr><td><p style="margin-left:-50px;font-size:25px!important;">is a bonafied student of this school studying class <i>'
					+ classname
					+ '</i><div style="width:25%;border-bottom: 1px dashed;display: block; margin-left:78%; margin-top: -10px;"></div></p></td></tr>'
					+ '<tr><td><p style="margin-left:-50px;font-size:25px!important;">during the academic year &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
					+ accyear
					+ '</i><div style="width:65%;border-bottom: 1px dashed;display: block;margin-left:35%; margin-top: -10px;"></div></p></td></tr><tr><td colspan="2"><p style="margin-left:-50px; font-size:25px!important;">His/Her date of birth as per the school admission register is  &nbsp;'
					+ dob
					+ '</i><div style="width:30%;border-bottom: 1px dashed;display: block; margin-left:-8%; margin-top: -10px;"></div></p></td></tr></table><div><br><br>'
					+ '<table width="100%" id="spacesize"><tr><th></th></tr></br><tr><th></th></tr></br><tr><th></th></tr></br><tr><th></th></tr></br><tr><th style="width: 50%;font-size:25px!important;"><p style="margin-left:10px;">Seal&nbsp&nbsp</p></th><th style="width: 60%;text-align:center;font-size:25px!important;">Principal</th></tr></table>'
