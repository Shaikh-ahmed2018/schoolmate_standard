$(document).ready(function() {
					...
					$('#classname').change(function() {
						...
						if(classname==""){
							...
							getsstudentdetails($("#locationname").val(),$("#Acyearid").val(),classname,$("#studSectionId").val());
						}
					...
					$("#rollNoGenerator").click(function(){
						if($("#classname").val() == ""){
							...
							$(".validateTips").text("Select the class");
							$("#classname").css({'border': '1px solid rgb(175, 44, 44)',
												 'background-color' : 'rgb(255, 247, 247)'
												});	
							$(".errormessagediv").delay(2000).fadeOut("slow");
						}else if($("#studSectionId").val() == ""){

							$(".errormessagediv").show();
							$(".validateTips").text("Select the division");
							$("#studSectionId").css({'border': '1px solid rgb(175, 44, 44)',
												 'background-color' : 'rgb(255, 247, 247)'
												});	
							$(".errormessagediv").delay(2000).fadeOut("slow");
...
datalist = {
			...
			"accyear" :accyear,
			"classId" :classname,
			"sectionid" :$("#studSectionId").val(),
		}, $.ajax({
			type : 'POST',
			url : "menuslist.html?method=getStudentListBySectionForROllNo",
			data : datalist,
			beforeSend: function(){
				$("#loder").show();
			},
			...
			success : function(response) {
				...
				$("#rollNoGenerator").click(function(){
					if($("#OrderingCondition #order1 .field").val()!=""){
						...
						$(".errormessagediv").delay(2000).fadeOut("slow");
						$("#OrderingCondition #order1 .field").css({'border': '1px solid rgb(175, 44, 44)',
							 'background-color' : 'rgb(255, 247, 247)'
									});
...
function rollNoGeneration(locationid,accyear,classname,sectionid){
	...
	if($("#order3 .field").val()!=""){
	 ...
	 $.ajax({
		 type:'POST',
		 url:'menuslist.html?method=rollNoGeneration',
		data:jsonOBj,
		beforeSend: function(){
			$("#loder").show();
		},
	...
	$.ajax({
		...
		success : function(response) {
			...
			if(result.status=="true"){
				...
				setTimeout(function(){
					window.location.href="menuslist.html?method=generateRollNo";
				},5000);
			}
			else{
				$("#loder").hide();
...
function HideError() 
{
	...
	$("#OrderingCondition #order1 .field1").css({'border-color':'rgb(218, 206, 206);'});    
	$("#OrderingCondition #order1 .field1").css({'background-color':'rgb(255, 255, 255)'});
