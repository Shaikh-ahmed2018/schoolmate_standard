...
function getIdCardStreamList(){
	...
	$.ajax({
		url : "menuslist.html?method=TransportIDCardDesignList",
		data:dataList,
		async : false,
		success : function(data) {
			...
			for ( var j = 0; j < result.streamList.length; j++) {
				...
				$('#allstudent tbody').append("<tr class='"+ result.streamList[j].academicYearCode+" "+ result.streamList[j].locationId+" "+ result.streamList[j].streamId+"'><td>"+ result.streamList[j].sno+ "</td>" +
						"<td>"+ result.streamList[j].academicYear+ "</td>" +
						"<td>"+ result.streamList[j].locationName+ "</td>" +
						"<td>"+ result.streamList[j].streamName+ "</td>" +
						"<td>"+ result.streamList[j].templateName+ "</td>" +
						"<td class='"+ result.streamList[j].status+ "'>"+ result.streamList[j].status+ "</td>" +
						"</tr>" +
						"");
...
function rowClickable(){
	$("#allstudent tbody tr").click(function(){
		...
		window.location.href="menuslist.html?method=PrintTransportIDCard&accyear="+$(this).attr("class").split(" ")[0]+"&schoolId="+$(this).attr("class").split(" ")[1]+"&streamId="+$(this).attr("class").split(" ")[2];
