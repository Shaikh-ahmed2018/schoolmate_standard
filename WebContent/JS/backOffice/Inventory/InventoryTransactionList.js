		...
		.ready(
				...
				$("#item_type").change(function(){
						...
						$.ajax({
							...
							type : "POST",
							url : "menuslist.html?method=getItemnameByItemtype",
						...
						$('#issued_quantity').on('change',function (){
							...
							$.ajax({
								...
								type : "POST",
								url : "menuslist.html?method=getAvailableQuantity",
								data : data,
								async : false,
					...
					$('#saveid').click(function() {
						...
						setTimeout(function(){
							window.location.href="menuslist.html?method=TransactionList";
						},2000);
					}
							else{
								$(".errormessagediv").show();
					...
					$("#edit").click(function(){
						 ...
						 if (transactionItem != undefined){
							...
							if(status !="returned"){
								window.location.href = "menuslist.html?method=EditTransactionDetails&transactionItemId="
									+ transactionItemId;
							}else {
										$(".errormessagediv").show();
										$(".validateTips")
					...
					if(editPage == "EditTransactionDetails&transactionItemId"){
						...
						var transactionItemId=window.location.href.substr(window.location.href.lastIndexOf("/") +1).split("=")[2]
						 $.ajax({
							 ...
							 type: 'POST', 
							 url: "menuslist.html?method=singleItemDetails",
							 data:{'transactionItemId':transactionItemId },
