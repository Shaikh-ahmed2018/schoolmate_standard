...
$("#allstudent tbody tr").click(function(){
	...
	window.location.href="feecollection.html?method=setFeeRequestStudentWise&student_id="+student_Id[0]+
	"&classid="+classid[1]+"&sectionid="+sectionid[1]+"&loc_id="+loc_id[1]+"&acy_id="+acy_id[0]+
	"&historylocId="+$("#locationname").val()+"&historyacademicId="+$("#Acyearid").val()+"&historyclassname="+$("#classname").val()
	+"&historysectionid="+$("#sectionid").val()+"&historysearchvalue="+$("#searchvalue").val()+
	"&historyisConcession="+$("#isConcession").val();
...
$("#ShowPerPage").change(function(){
  ...
  if($("#historylocId").val()!="" || $("#historyacademicId").val()!="" || $("#historysearchvalue").val()!="" || $("#historyisConcession").val()=="Y"){
	  ...
	  $("#isConcession").val($("#historyisConcession").val());
			...
			success : function(response) {
				...
				var result = $.parseJSON(response);
					...
					$("#allstudent tbody tr").click(function(){
						...
						window.location.href="feecollection.html?method=setFeeRequestStudentWise&student_id="+student_Id[0]+"&classid="+classid[1]+
						"&sectionid="+sectionid[1]+"&loc_id="+student_Id[2]+"&acy_id="+student_Id[1]
						"&historylocId="+$("#locationname").val()+"&historyacademicId="+$("#Acyearid").val()+"&historyclassname="+$("#classname").val()
						+"&historysectionid="+$("#sectionid").val()+"&historysearchvalue="+$("#searchvalue").val()+
						"&historyisConcession="+$("#isConcession").val();
					});
...
function getStudentListBySection(locationid,accyear,classname,sectionid){
	...
	datalist = {
			...
			"isConcession":$("#isConcession").val(),
			"searchvalue":$("#searchvalue").val()
		}, $.ajax({
			type : 'POST',
			url : "menuslist.html?method=getStudentListByFeeRequestCancel",
			data : datalist,
			beforeSend: function(){
				$("#loder").show();
			},
			...
			success : function(response) {
				...
				var result = $.parseJSON(response);
					...
					$("#allstudent tbody tr").click(function(){
						...
						window.location.href="feecollection.html?method=setFeeRequestStudentWise&student_id="+student_Id[0]+"&classid="+classid[1]+"&sectionid="+sectionid[1]+"&loc_id="+student_Id[2]+"&acy_id="+student_Id[1];
				});
				}else{
					$("#allstudent tbody").append("<tr><td colspan='5'>No Records Found</td></tr>");
				}
