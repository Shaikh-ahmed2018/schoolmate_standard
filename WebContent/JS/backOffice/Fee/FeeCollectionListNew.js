$(document).ready(function(){
	...
	$("#resetbtn").click(function(){
		...
		getStudentListByJs("no","no",$("#classname").val(),$("#sectionid").val(),$("#searchvalue").val());
	...
	$("#search").click(function(){
		...
		if($("#searchvalue").val().trim()!="" && $("#searchvalue").val().trim()!=null){
	...
	$("#allstudent tbody tr").click(function(){
		...
		var classId=studentIds[3];
		var specId=studentIds[4];
		window.location.href = "feeCollectionNew.html?method=feeCollectionStudentWise&student="+studentId+
		"&loc_id="+location+"&academicYear="+academicYear+"&classId="+classId+"&spec="+specId
		+"&historylocId="+$("#locationname").val()+"&historyacademicId="+$("#Acyearid").val()+
		"&historyclassname="+$("#classname").val()+"&historysectionId="+$("#sectionid").val()+
...
function handle(event){
	if(event.keyCode==13){
		...
		window.location.href="feeCollectionNew.html?method=feeCollection&stuId="+stuId;
	}
}
...
function getStudentListByJs(locationId,academicYear,classId,divisionId,searchTerm){
	...
	$.ajax({
		...
		type : 'POST',
		url : "feeCollectionNew.html?method=studentListByJs",
		data : datalist,
		beforeSend: function(){
			$("#loder").show();
		},
		success : function(response) {
			...
			if(leng>0){
				for( i=0;i<leng;i++){
					...
					$('#allstudent tbody').append("<tr class='"+rel.studentId+" "+rel.academicYearId+" "+rel.locationId+" "+rel.classDetailId+" "+rel.specilization+"'>" +
							"<td>"+(i+1)+"</td>" +
