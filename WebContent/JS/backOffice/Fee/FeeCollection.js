...
$(document).ready(function() {
					...
					$("table.pending tbody tr").each(function(){
						var feeamount=0;
						if(!(isNaN($(this).find(".houtStandingAmount").val()))){
						var houtStandingAmount=$(this).find(".houtStandingAmount").val();
						
						feeamount=houtStandingAmount;
						
					
						$(this).find("input.feeAmount").val(feeamount);
						}
					
					
					if(!(isNaN($(this).find(".feeBalanceAmount").val())))
					amount+=parseFloat($(this).find(".feeAmount").val());
				
							});	
					
					$("table.notPaid tbody tr").each(function(){
						if($(this).find(".isconcession").val()=="N"){
							$(this).find("input.concession").val("0.0");
						}
				...
				$("table.pending td#tot_actual_amt").text((amount).toFixed(2));
					...
					.click(
							function() {
								...
								window.location.href = "menuslist.html?method=downloadfeecollectionXLS";
								
							});
					$("#pdfDownload").click(function(){
						
	        			window.location.href = "menuslist.html?method=downloadfeecollectionPDF";
					...
					.click(
							function() {
								...
								if (count > 1 || count == 0) {
									...
									if(editcode=="Not Paid"){
										$('.errormessagediv').show();
										$('.validateTips').text("Go For First Payment");
									}
									else{
									window.location.href = "feecollection.html?method=editFeeCollectionDetails&FeeCode="
											+ feecode;
									}
								}
								...
								$.ajax({
									...
									success : function(data) {
										...
										setTimeout(function(){
											 ...
											 window.location.href ='menuslist.html?method=feeCollection';
