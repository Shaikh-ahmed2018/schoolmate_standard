...
function getAllSchool(){
	$.ajax({
		type:'POST',
		url:'menuslist.html?method=getAllSchool',
		async:false,
		success:function(data){
			...
			$(".school").find("option").not("option[value="+$("#hschoolLocation").val()+"],option[value='']").remove();
		}
	});
}
function getAllClassList(pointer,location){
	$.ajax({
		type:'POST',
		url:'menuslist.html?method=getClassList',
...
function getChangedTermList(location,accyear){
	$.ajax({
		type:'POST',
		url:'menuslist.html?method=getTermList',
...
function getAccyearList(){
	$.ajax({
		type:'POST',
		url:'menuslist.html?method=getAccyearList',
...
function getFineList(accyear){
	$.ajax({
		type:"POST",
		url:"menuslist.html?method=fineConfigurationByAccyear",
		...
		success:function(response){
			...
			$("#allstudent tbody").append("<tr>" +
					...
					"<td><input type='text' class='date' value='' readonly='readonly' placeholder='SELECT DATE' /></td>"+
...
function addingrow(){
	var len=$("table#allstudent tbody tr").length;
	$("table#allstudent tbody tr:last-child .amount").keyup(function(){
		
		if($(this).val()>0 && $(this).val().length==1 && len ==$("table#allstudent tbody tr").length){
			
			$("#allstudent tbody").append("<tr>" +
					
					"<td><select  class='accyear yr-"+len+"' ></select><input type='hidden' value='' class='haccyearid' /></td>" +
					"<td><select  class='school sc-"+len+"'' ><option value=''>------SELECT------</option></select><input type='hidden' value='' class='hlocationid' /></td>" +
					"<td><select class='ClassList' ><option value=''>------SELECT------</option></select><input type='hidden' value='' class='hclassnameid' /></td>"+
					"<td><select class='termlist' ><option value=''>------SELECT------</option></select><input type='hidden' value='' class='htermlist' /></td>"+
					"<td><input type='text' class='date' value='' readonly='readonly' placeholder='SELECT DATE' /></td>"+
					"<td><input type='number' class='amount' value='' /></td>" +
			...
			$.ajax({
				...
				success:function(data){
					...
					for(var i=0;i<result.Accyear.length;i++){
						$("select.yr-"+len).append("<option value='"+result.Accyear[i].accId+"'>"+result.Accyear[i].accName+"</option>");
					}
			...
			$.ajax({
				...
				success:function(data){
					var result=$.parseJSON(data);
					$("select.sc-"+len).empty();
					$("select.sc-"+len).append("<option value=''>------SELECT------</option>");
					for(var i=0;i<result.schoolName.length;i++){
						$("select.sc-"+len).append("<option value='"+result.schoolName[i].locationId+"'>"+result.schoolName[i].locationName+"</option>");
					}
					
					$("select.sc-"+len).find("option").not("option[value="+$("#hschoolLocation").val()+"],option[value='']").remove();
				}
			});
			$(".accyear").change(function(){
				getChangedTermList($(this).closest("tr").find(".school"),$(this))
...
function dateRange(accyear){
	...
	$.ajax({
		...
		success:function(response){
			...
			dateRange.push(result.startDate.split(",")[0]);
			dateRange.val(result.startDate.split(",")[1]);
		}
