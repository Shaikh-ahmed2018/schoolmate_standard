...
$(document).ready(function() {
	...
	if($("#specilization").val('XI')){
		 $('select[name^="qualifying"] option[value="X"]').attr("selected","selected");
		 
		 $('select[name^="qualifying"] option[value="XII"]').attr("hidden","hidden");
		 $('select[name^="qualifying"] option[value="XI"]').attr("hidden","hidden");
	}else if($("#specilization").val('XII')){
		 $('select[name^="qualifying"] option[value="XI"]').attr("selected","selected");
		 
		 $('select[name^="qualifying"] option[value="XII"]').attr("hidden","hidden");
		 $('select[name^="qualifying"] option[value="X"]').attr("hidden","hidden");
	}
	...
	var enquiryId=window.location.href.substring(window.location.href.lastIndexOf("=")+1);
	...
	$("#dialog").dialog({		
		...
		buttons : {
			"Yes" : function()
			{
				...
				{
					$("#formstudent").submit();
					$("#formstudent").attr('action','parentrequiresappointment.html?method=saveThirdAdmissionApplication');
...
function getClassName() {
	$.ajax({
		type : 'POST',
		url : "parentrequiresappointment.html?method=getClassDetailSrSecondary",
		async : false,
		success : function(response) {
			var result = $.parseJSON(response);
			$('#classId').html("");
...
function getClassSpecilization(){
	...
	$.ajax({
		type : 'POST',
		url : "specialization.html?method=getSpecializationOnClassWithoutLocId",
		data: data,
		async : false,
		success : function(response) {
			var result = $.parseJSON(response);
...
function Validate() {
	...
	var castecategoryid=$("#casteCategoryId");
	...
	var qualifyingId=$('#qualifyingId').val();
	...
	}else if (!studentLastName.val().match(/[A-Za-z ]+$/i)) {

		$('.errormessagediv').show();
		$('.validateTips').text("Field Required Valid Last Name.");
		$("#studentFirstNameId").focus();
		document.getElementById("studentLastNameId").style.border = "1px solid #AF2C2C";
		document.getElementById("studentLastNameId").style.backgroundColor = "#FFF7F7";
		setTimeout(function() {
			$('#errorhover').fadeOut();
	...
	else if (castecategoryid.val() == "" || castecategoryid.val() == null){
		$(".errormessagediv").show();
	...
	else if (castecategoryid.val() == 'Other' && othercastecategory.val() == "") {
		$('.errormessagediv').show();
		$('.validateTips').text("Field Required Other Caste Category.");
		$("#othercastecategoryId").focus();
		document.getElementById("othercastecategoryId").style.border = "1px solid #AF2C2C";
		document.getElementById("othercastecategoryId").style.backgroundColor = "#FFF7F7";
		setTimeout(function() {
			$('#errorhover').fadeOut();
			document.getElementById("othercastecategoryId").style.border = "1px solid #ccc";
			document.getElementById("othercastecategoryId").style.backgroundColor = "#fff";
		}, 500);
	...
	else if (qualifyingId == "" || qualifyingId == null){

		$(".errormessagediv").show();
		$(".validateTips").text("Field Required Name of Qualifying Examination.");
		$("#qualifyingId").focus();
		document.getElementById("qualifyingId").style.border = "1px solid #AF2C2C";
		document.getElementById("qualifyingId").style.backgroundColor = "#FFF7F7";
		setTimeout(function() {
			$('#errorhover').fadeOut();
			document.getElementById("qualifyingId").style.border = "1px solid #ccc";
			document.getElementById("qualifyingId").style.backgroundColor = "#fff";
		}, 1000);
	...
	else if (specilization == "" || specilization == null){
...
function viewAdmissionStudentDetails(admissionStudentRecord) {
	...
	var parents=$('#preferedphnoId').val();
	if(parents == "father"){
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (result.validatenostatus == "true") {
				...
				window.location.href="menuslist.html?method=OnlinereturnFormPage";
				status = true;
			} else {
				$('.errormessagediv').css({
					'display' : 'none'
