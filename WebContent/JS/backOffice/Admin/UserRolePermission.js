...
//Library Issue Details	
	...
	$("#PreviewPermission").click(function(){
		...
		$(".col-md-12 #allstudents tbody tr td").each(function(){
		var visibleRow=$(this).find("input[type='checkbox']:checked").attr("id");
		if(visibleRow!=undefined){
			$(".modal #allstudent #"+visibleRow).parent("td").parent("tr").show();
			$(".modal #allstudent #"+visibleRow).prop("checked",true);
		}
		});
		$(".modal").css({
	...
	$('#savePermission').click(function(){
		...
		setTimeout(function(){
			...
			var dataArray={
					"roleCode":$('#roleId').val(),
					"permissionCode":permissionCode.join(),
					"permissionShortName":permissionShortName.join(),
					"isPermissionAllowed":isPermissionAllowed.join(),
					};
			...
			$.ajax({
					...
					success:function(response){
						...
						setTimeout(function(){
							 ...
							 window.location.href="menuslist.html?method=getUserRolePermission";
	...
	$('#roleId').change(function(){
			...
			success:function(data){
				...
				for(var i=0;i<result.permission.length;i++){
					...
					if(isApplicable=="true"){
						...
						$('.col-md-12 #'+id).prop('checked',true);
						$('.col-md-12 #'+id).parent().css({'color':'blue'});
					}
					else{
		...
		$("#allstudents tbody tr").each(function(){
			if($(this).find(".permission").length==$(this).find(".permission:checked").length){
				$(this).find(".submoduleCheck").prop("checked",true);
			}
		});
		if($(".submoduleCheck").length==$(".submoduleCheck:checked").length){
