...
function checkLeaveCount(){
	...
	$.ajax({
		...
		success : function(response) {
			...
			var monthleaveCount = (result.tot_leaves[0].leaveCount);
			
			if(monthleaveCount == 0){
				$('.errormessagediv').show();
				$('.validateTips').text("You don't have leaves for the selected month");
				setTimeout(function() {
					$('#startsessionDay').val("");
					$('#endsessionDay').val("");
					$('#totalleaves').val("");
