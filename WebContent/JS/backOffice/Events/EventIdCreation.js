		...
		$("#saveChanges").click(function(){
			...
			$(".section div[id]").each(function(){
				...
				if($(this).attr("style[width]")!=undefined){
					...
					var carryCss1="#"+$(this).attr("id")+"{background-color:"+rgb2hex($("#"+$(this).attr("id")).css('background-color'))+"}";
			...
			if($('#lfontval').val()!="0px"){
				...
				var css5="#nametext{font-size:"+$('#lfontval').val()+"}.studentdetails span{font-size:"+$('#lfontval').val()+"}.addressdetails span{font-size:"+$('#lfontval').val()+"}.phonedetails span{font-size:"+$('#lfontval').val()+"}";
				
				cssgroup.push(css5);
			}
			else{
				var carryCss5="#nametext{font-size:"+$('#nametext').css('font-size')+"}.studentdetails span{font-size:"+$('#nametext').css('font-size')+"}.addressdetails span{font-size:"+$('#nametext').css('font-size')+"}.phonedetails span{font-size:"+$('#nametext').css('font-size')+"}";
			...
			var mainCss=cssgroup.toString().replace(/,/g,"");
			 $.ajax({
				 type: 'POST', 
				  url: "EventsMenu.html?method=EvntIdDesign",
...
function getStudentIdDesignList(){
	$.ajax({
		type:"POST",
		url:"menuslist.html?method=getStudentIdDesignList",
		async:false,
...
function saveBackground(){
	...
	var file=$("#backgroundfile")[0].files[0];
		...
		$.ajax({
			url:'menuslist.html?method=saveLayoutImage',
			cache:false,
			type: 'POST',
			data:formdata,
			contentType: false,
